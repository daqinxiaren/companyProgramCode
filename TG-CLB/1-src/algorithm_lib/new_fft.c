...
#else /*LINUX ARM*/
	fftinput = (fftw_complex*)algor_malloc(sizeof(fftw_complex)*4096);
	if (NULL == fftinput)
	{
		printf("algor_malloc fftinput failed !!!!!\n");
		return -1;
	}
	fftoutput = (fftw_complex*)algor_malloc(sizeof(fftw_complex)*4096);
	if (NULL == fftoutput)
	{
		printf("algor_malloc fftoutput failed !!!!!\n");
		return -1;
	}
#endif
	return 0;
