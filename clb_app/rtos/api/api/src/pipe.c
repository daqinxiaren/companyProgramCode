...
static int pipe_fops_open(struct dfs_fd *fd)
{
    ...
    device = &(pipe->parent);
    rt_mutex_take(&(pipe->lock), WAITING_FOREVER);

    if (device->ref_count == 0)
    {
        pipe->fifo = rt_ringbuffer_create(pipe->bufsz);
        if (pipe->fifo == OS_NULL)
        {
            rc = -OS_ENOMEM;
            goto __exit;
        }
...
static int pipe_fops_close(struct dfs_fd *fd)
{
    ...
    if (device->ref_count == 0 && pipe->is_named == OS_FALSE)
    {
...
rt_err_t  rt_pipe_close  (rt_device_t device)
{
    ...
    if (device == OS_NULL) return -OS_EINVAL;
    rt_mutex_take(&(pipe->lock), WAITING_FOREVER);
...
rt_pipe_t *rt_pipe_create(const char *name, int bufsz)
{
    ...
    if (pipe == OS_NULL) return OS_NULL;

    rt_memset(pipe, 0, sizeof(rt_pipe_t));
    pipe->is_named = OS_TRUE; /* initialize as a named pipe */
    rt_mutex_init(&(pipe->lock), name, IPC_FLAG_FIFO);
    rt_wqueue_init(&(pipe->reader_queue));
    rt_wqueue_init(&(pipe->writer_queue));

    OS_ASSERT(bufsz < 0xFFFF);
    pipe->bufsz = bufsz;

    dev = &(pipe->parent);
    dev->type = DEVICE_CLASS_PIPE;
#ifdef ENABLE_DEVICE_OPS
    dev->ops         = &pipe_ops;
#else
    dev->init        = OS_NULL;
    ...
    if (rt_device_register(&(pipe->parent), name, DEVICE_FLAG_RDWR | DEVICE_FLAG_REMOVABLE) != 0)
    {
        rt_free(pipe);
        return OS_NULL;
    }
...
int rt_pipe_delete(const char *name)
{
    ...
    if (device)
    {
        if (device->type == DEVICE_CLASS_PIPE)
        {
            rt_pipe_t *pipe;
...
int pipe(int fildes[2])
{
    ...
    pipe = rt_pipe_create(dname, PIPE_BUFFER_SIZE);
    if (pipe == OS_NULL)
    {
        return -1;
    }

    pipe->is_named = OS_FALSE; /* unamed pipe */
...
int mkfifo(const char *path, mode_t mode)
{
    ...
    pipe = rt_pipe_create(path, PIPE_BUFFER_SIZE);
    if (pipe == OS_NULL)
    {
        return -1;
    }
