...
OS_MODE_EXPORT(rt_rbb_blk_alloc);
...
{
    OS_ASSERT(block);
    OS_ASSERT(block->status == RBB_BLK_INITED);

    block->status = RBB_BLK_PUT;
}
OS_MODE_EXPORT(rt_rbb_blk_put);
...
OS_MODE_EXPORT(rt_rbb_blk_size);
...
void rt_rbb_blk_free(rt_rbb_t rbb, rt_rbb_blk_t block)
{
    ...
    OS_ASSERT(rbb);
    OS_ASSERT(block);
    OS_ASSERT(block->status != RBB_BLK_UNUSED);
...
OS_MODE_EXPORT(rt_rbb_blk_free);
...
OS_MODE_EXPORT(rt_rbb_blk_queue_get);
...
OS_MODE_EXPORT(rt_rbb_blk_queue_len);
...
OS_MODE_EXPORT(rt_rbb_blk_queue_buf);
...
OS_MODE_EXPORT(rt_rbb_blk_queue_free);
...
OS_MODE_EXPORT(rt_rbb_next_blk_queue_len);
...
OS_MODE_EXPORT(rt_rbb_get_buf_size);
