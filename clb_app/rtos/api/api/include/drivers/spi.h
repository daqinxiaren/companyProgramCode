 ...
 *    on a falling edge.
 */
#define DEV_SPI_CPHA     (1<<0)                             /* bit[0]:CPHA, clock phase */
#define DEV_SPI_CPOL     (1<<1)                             /* bit[1]:CPOL, clock polarity */

#define DEV_SPI_LSB      (0<<2)                             /* bit[2]: 0-LSB */
#define DEV_SPI_MSB      (1<<2)                             /* bit[2]: 1-MSB */

#define DEV_SPI_MASTER   (0<<3)                             /* SPI master device */
#define DEV_SPI_SLAVE    (1<<3)                             /* SPI slave device */

#define DEV_SPI_CS_HIGH  (1<<4)                             /* Chipselect active high */
#define DEV_SPI_NO_CS    (1<<5)                             /* No chipselect */
#define DEV_SPI_3WIRE    (1<<6)                             /* SI/SO pin shared */
#define DEV_SPI_READY    (1<<7)                             /* Slave pulls low to pause */

#define DEV_SPI_MODE_MASK    (DEV_SPI_CPHA | DEV_SPI_CPOL | DEV_SPI_MSB | DEV_SPI_SLAVE | DEV_SPI_CS_HIGH | DEV_SPI_NO_CS | DEV_SPI_3WIRE | DEV_SPI_READY)

#define DEV_SPI_MODE_0       (0 | 0)                        /* CPOL = 0, CPHA = 0 */
#define DEV_SPI_MODE_1       (0 | DEV_SPI_CPHA)              /* CPOL = 0, CPHA = 1 */
#define DEV_SPI_MODE_2       (DEV_SPI_CPOL | 0)              /* CPOL = 1, CPHA = 0 */
#define DEV_SPI_MODE_3       (DEV_SPI_CPOL | DEV_SPI_CPHA)    /* CPOL = 1, CPHA = 1 */

#define DEV_SPI_BUS_MODE_SPI         (1<<0)
#define DEV_SPI_BUS_MODE_QSPI        (1<<1)
 ...
 *
 * @return OS_EOK on taken SPI bus successfully. others on taken SPI bus failed.
 */
 ...
 * @param device the SPI device attached to SPI bus
 *
 * @return OS_EOK on release SPI bus successfully.
 */
 ...
 * @param device the SPI device attached to SPI bus
 *
 * @return OS_EOK on release SPI bus successfully. others on taken SPI bus failed.
 */
 ...
 * @param device the SPI device attached to SPI bus
 *
 * @return OS_EOK on release SPI device successfully.
 */
 ...
 *
 * @return OS_NULL if transmits message list successfully,
...
rt_inline rt_size_t rt_spi_recv(struct rt_spi_device *device,
                                ...
                                rt_size_t             length)
{
    return rt_spi_transfer(device, OS_NULL, recv_buf, length);
}
...
rt_inline rt_size_t rt_spi_send(struct rt_spi_device *device,
                                ...
                                rt_size_t             length)
{
    return rt_spi_transfer(device, send_buf, OS_NULL, length);
}
