...
#define MTD_NAND_DEVICE(device)  ((struct rt_mtd_nand_device*)(device))

#define MTD_EOK          0     /* NO error */
#define MTD_EECC         101   /* ECC error */
#define MTD_EBUSY        102   /* hardware busy */
#define MTD_EIO          103   /* generic IO issue */
#define MTD_ENOMEM       104   /* out of memory */
#define MTD_ESRC         105   /* source issue */
#define MTD_EECC_CORRECT 106   /* ECC error but correct */
...
rt_inline rt_err_t rt_mtd_nand_read(
    ...
    rt_uint8_t *spare, rt_uint32_t spare_len)
{
    OS_ASSERT(device->ops->read_page);
...
rt_inline rt_err_t rt_mtd_nand_write(
    ...
    const rt_uint8_t *spare, rt_uint32_t spare_len)
{
    OS_ASSERT(device->ops->write_page);
...
{
    OS_ASSERT(device->ops->move_page);
...
{
    OS_ASSERT(device->ops->erase_block);
