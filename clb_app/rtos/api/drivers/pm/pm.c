...
static rt_uint8_t _pm_default_sleep = POWER_DEFAULT_SLEEP_MODE;

/* default deepsleep mode : tick-less mode */
static rt_uint8_t _pm_default_deepsleep = POWER_DEFAULT_DEEPSLEEP_MODE;

static struct rt_pm_notify _pm_notify;
static rt_uint8_t _pm_init_flag = 0;

#define POWER_TICKLESS_THRESH (2)

OS_WEAK rt_uint32_t rt_pm_enter_critical(rt_uint8_t sleep_mode)
{
    return rt_hw_interrupt_disable();
}

OS_WEAK void rt_pm_exit_critical(rt_uint32_t ctx, rt_uint8_t sleep_mode)
{
...
static int _pm_device_suspend(rt_uint8_t mode)
{
    ...
    {
        if (_pm.device_pm[index].ops->suspend != OS_NULL)
        {
...
static void _pm_device_resume(rt_uint8_t mode)
{
    ...
    for (index = 0; index < _pm.device_pm_number; index++)
    {
        if (_pm.device_pm[index].ops->resume != OS_NULL)
        {
...
static void _pm_device_frequency_change(rt_uint8_t mode)
{
    ...
    for (index = 0; index < _pm.device_pm_number; index ++)
    {
        if (_pm.device_pm[index].ops->frequency_change != OS_NULL)
...
static void _pm_frequency_scaling(struct rt_pm *pm)
{
    ...
    if (pm->flags & POWER_FREQUENCY_PENDING)
    {
        ...
        pm->flags &= ~POWER_FREQUENCY_PENDING;
...
static rt_uint8_t _pm_select_sleep_mode(struct rt_pm *pm)
{
    ...
    for (index = POWER_SLEEP_MODE_NONE; index < POWER_SLEEP_MODE_MAX; index ++)
    {
        if (pm->modes[index])
        {
            mode = index;
...
void rt_pm_module_delay_sleep(rt_uint8_t module_id, rt_tick_t timeout)
{
    ...
    if (module_id > (POWER_MODULE_MAX_ID - 1))
        return;

    level = rt_hw_interrupt_disable();
    pm = &_pm;
    pm->module_status[module_id].busy_flag = OS_TRUE;
...
static rt_bool_t _pm_device_check_idle(void)
{
    ...
    pm = &_pm;
    for (int i = 0; i < POWER_MODULE_MAX_ID; i++)
    {
        if (pm->module_status[i].busy_flag == OS_TRUE)
        {
            if (rt_tick_get() - pm->module_status[i].start_time > pm->module_status[i].timeout)
            {
                pm->module_status[i].busy_flag = OS_FALSE;
...
static void _pm_change_sleep_mode(struct rt_pm *pm)
{
    ...
    /* module busy request */
    if (_pm_device_check_idle() == OS_FALSE)
    {
        pm->ops->sleep(pm, POWER_SLEEP_MODE_NONE);
        rt_pm_exit_critical(level, pm->sleep_mode);
        return;
    }

    if (_pm.sleep_mode == POWER_SLEEP_MODE_NONE)
    {
        pm->ops->sleep(pm, POWER_SLEEP_MODE_NONE);
        rt_pm_exit_critical(level, pm->sleep_mode);
    }
    else
    {
        ...
        if (_pm_notify.notify)
            _pm_notify.notify(POWER_ENTER_SLEEP, pm->sleep_mode, _pm_notify.data);
        ...
        if (ret != OS_EOK)
        {
            ...
            if (_pm_notify.notify)
                _pm_notify.notify(POWER_EXIT_SLEEP, pm->sleep_mode, _pm_notify.data);
            rt_pm_exit_critical(level, pm->sleep_mode);
        ...
        if (pm->timer_mask & (0x01 << pm->sleep_mode))
        {
            ...
            if (timeout_tick == TICK_MAX)
            {
                if (pm->ops->timer_start)
                {
                    pm->ops->timer_start(pm, TICK_MAX);
                }
            }
            else
            {
                timeout_tick = timeout_tick - rt_tick_get();
                if (timeout_tick < POWER_TICKLESS_THRESH)
                {
                    pm->sleep_mode = POWER_SLEEP_MODE_IDLE;
                }
                else
                {
...
void rt_pm_request(rt_uint8_t mode)
{
    ...
    if (mode > (POWER_SLEEP_MODE_MAX - 1))
        return;
...
void rt_pm_device_register(struct rt_device *device, const struct rt_device_pm_ops *ops)
{
    ...
    device_pm = (struct rt_device_pm *)KERNEL_REALLOC(_pm.device_pm,
                (_pm.device_pm_number + 1) * sizeof(struct rt_device_pm));
    if (device_pm != OS_NULL)
    {
...
void rt_pm_device_unregister(struct rt_device *device)
{
    ...
    for (index = 0; index < _pm.device_pm_number; index ++)
    {
        if (_pm.device_pm[index].device == device)
        {
            ...
            _pm.device_pm[_pm.device_pm_number - 1].device = OS_NULL;
            _pm.device_pm[_pm.device_pm_number - 1].ops = OS_NULL;
                                      ...
                                      void       *args)
{
    ...
    switch (cmd)
    {
    case POWER_DEVICE_CTRL_REQUEST:
        mode = (rt_uint32_t)args;
        rt_pm_request(mode);
        break;

    case POWER_DEVICE_CTRL_RELEASE:
        mode = (rt_uint32_t)args;
        rt_pm_release(mode);
        break;
    }
                       ...
                       void                   *user_data)
{
    ...
    rt_device_register(device, "pm", DEVICE_FLAG_RDWR);
    ...
    pm->module_status[POWER_POWER_ID].req_status = 1;
    pm->run_mode   = POWER_DEFAULT_RUN_MODE;
    pm->timer_mask = timer_mask;

    pm->ops = ops;

    pm->device_pm = OS_NULL;
...
static const char *_pm_sleep_str[] = POWER_SLEEP_MODE_NAMES;
static const char *_pm_run_str[] = POWER_RUN_MODE_NAMES;
...
static void rt_pm_dump_status(void)
{
    ...
    rt_kprintf("+-----------------------+---------+-------+\n");
    for (index = 0; index < POWER_SLEEP_MODE_MAX; index ++)
    {
        int has_timer = 0;
    ...
    rt_kprintf("+--------+------+------------+-----------+\n");
    for (index = 0; index < POWER_MODULE_MAX_ID; index ++)
    {
        if ((pm->module_status[index].busy_flag == OS_TRUE) ||
