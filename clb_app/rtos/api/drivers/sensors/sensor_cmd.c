...
static void sensor_fifo_rx_entry(void *parameter)
{
    ...
    struct rt_sensor_data *data = OS_NULL;
    struct rt_sensor_info info;
    rt_size_t res, i;
    
    rt_device_control(dev, DEV_SENSOR_CTRL_GET_INFO, &info);
    ...
    while (1)
    {
        rt_sem_take(sensor_rx_sem, WAITING_FOREVER);
...
{
    static rt_thread_t tid1 = OS_NULL;
    rt_device_t dev = OS_NULL;
    rt_sensor_t sensor;
    ...
    if (rt_device_open(dev, DEVICE_FLAG_FIFO_RX) != OS_EOK)
    {
    ...
    {
        sensor_rx_sem = rt_sem_create("sen_rx_sem", 0, IPC_FLAG_FIFO);
    }
    else
    {
    ...
    rt_device_control(dev, DEV_SENSOR_CTRL_SET_ODR, (void *)20);
}
...
static void sensor_int(int argc, char **argv)
{
    ...
    if (rt_device_open(dev, DEVICE_FLAG_INT_RX) != OS_EOK)
    {
        LOG_E("open device failed!");
        return;
    }
...
static void sensor_polling(int argc, char **argv)
{
    ...
    if (rt_device_open(dev, DEVICE_FLAG_RDWR) != OS_EOK)
    {
        LOG_E("open device failed!");
        return;
    }
    rt_device_control(dev, DEV_SENSOR_CTRL_SET_ODR, (void *)100);
...
{
    static rt_device_t dev = OS_NULL;
    struct rt_sensor_data data;
    rt_sensor_t sensor;
    rt_size_t res, i;
    rt_int32_t delay;
    ...
    else if (!strcmp(argv[1], "info"))
    {
        ...
        switch (info.vendor)
        {
            case DEV_SENSOR_VENDOR_UNKNOWN:
                rt_kprintf("vendor    :unknown vendor\n");
                break;
            case DEV_SENSOR_VENDOR_STM:
                rt_kprintf("vendor    :STMicroelectronics\n");
                break;
            case DEV_SENSOR_VENDOR_BOSCH:
                rt_kprintf("vendor    :Bosch\n");
                break;
            case DEV_SENSOR_VENDOR_INVENSENSE:
                rt_kprintf("vendor    :Invensense\n");
                break;
            case DEV_SENSOR_VENDOR_SEMTECH:
                rt_kprintf("vendor    :Semtech\n");
                break;
            case DEV_SENSOR_VENDOR_GOERTEK:
                rt_kprintf("vendor    :Goertek\n");
                break;
            case DEV_SENSOR_VENDOR_MIRAMEMS:
                rt_kprintf("vendor    :MiraMEMS\n");
                break;
            case DEV_SENSOR_VENDOR_DALLAS:
                rt_kprintf("vendor    :Dallas\n");
                break;
            case DEV_SENSOR_VENDOR_ASAIR:
                rt_kprintf("vendor    :Asair\n");
                break;
            case DEV_SENSOR_VENDOR_SHARP:
                rt_kprintf("vendor    :Sharp\n");
                break;
            case DEV_SENSOR_VENDOR_SENSIRION:
                rt_kprintf("vendor    :Sensirion\n");
                break;
            case DEV_SENSOR_VENDOR_TI:
                rt_kprintf("vendor    :Texas Instruments\n");
                break;
            case DEV_SENSOR_VENDOR_PLANTOWER:
                rt_kprintf("vendor    :Plantower\n");
                break;
            case DEV_SENSOR_VENDOR_AMS:
                rt_kprintf("vendor    :AMS\n");
                break;
            case DEV_SENSOR_VENDOR_MAXIM:
        ...
        switch (info.unit)
        {
            case DEV_SENSOR_UNIT_NONE:
                rt_kprintf("unit      :none\n");
                break;
            case DEV_SENSOR_UNIT_MG:
                rt_kprintf("unit      :mG\n");
                break;
            case DEV_SENSOR_UNIT_MDPS:
                rt_kprintf("unit      :mdps\n");
                break;
            case DEV_SENSOR_UNIT_MGAUSS:
                rt_kprintf("unit      :mGauss\n");
                break;
            case DEV_SENSOR_UNIT_LUX:
                rt_kprintf("unit      :lux\n");
                break;
            case DEV_SENSOR_UNIT_CM:
                rt_kprintf("unit      :cm\n");
                break;
            case DEV_SENSOR_UNIT_PA:
                rt_kprintf("unit      :pa\n");
                break;
            case DEV_SENSOR_UNIT_PERMILLAGE:
                rt_kprintf("unit      :permillage\n");
                break;
            case DEV_SENSOR_UNIT_DCELSIUS:
                rt_kprintf("unit      :Celsius\n");
                break;
            case DEV_SENSOR_UNIT_HZ:
                rt_kprintf("unit      :HZ\n");
                break;
            case DEV_SENSOR_UNIT_ONE:
                rt_kprintf("unit      :1\n");
                break;
            case DEV_SENSOR_UNIT_BPM:
                rt_kprintf("unit      :bpm\n");
                break;
            case DEV_SENSOR_UNIT_MM:
                rt_kprintf("unit      :mm\n");
                break;
            case DEV_SENSOR_UNIT_MN:
                rt_kprintf("unit      :mN\n");
                break;
            case DEV_SENSOR_UNIT_PPM:
                rt_kprintf("unit      :ppm\n");
                break;
            case DEV_SENSOR_UNIT_PPB:
                rt_kprintf("unit      :ppb\n");
                break;
        }
    ...
    else if (argc == 3)
    {
        if (!strcmp(argv[1], "probe"))
        {
            ...
            rt_device_control(dev, DEV_SENSOR_CTRL_GET_ID, &reg);
        ...
        {
            rt_device_control(dev, DEV_SENSOR_CTRL_SET_RANGE, (void *)atoi(argv[2]));
        }
        else if (!strcmp(argv[1], "sm"))
        {
            rt_device_control(dev, DEV_SENSOR_CTRL_SET_MODE, (void *)atoi(argv[2]));
        }
        else if (!strcmp(argv[1], "sp"))
        {
            rt_device_control(dev, DEV_SENSOR_CTRL_SET_POWER, (void *)atoi(argv[2]));
        }
        else if (!strcmp(argv[1], "sodr"))
        {
            rt_device_control(dev, DEV_SENSOR_CTRL_SET_ODR, (void *)atoi(argv[2]));
        }
        else
        {
