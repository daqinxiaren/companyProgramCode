...
#if defined(MTD_NAND_DEBUG) && defined(ENABLE_FINSH)
#include <finsh.h>
...
int mtd_nandid(const char *name)
{
    ...
    nand = MTD_NAND_DEVICE(rt_device_find(name));
    if (nand == OS_NULL)
    {
...
int mtd_nand_readoob(const char *name, int block, int page)
{
    ...
    /* calculate the page number */
    page = block * nand->pages_per_block + page;
    rt_mtd_nand_read(nand, page, OS_NULL, nand->page_size,
                     oob_ptr, nand->oob_size);
    mtd_dump_hex(oob_ptr, nand->oob_size);
...
#endif /* FINSH_ENABLE_MSH_ONLY */

#endif /* defined(MTD_NAND_DEBUG) && defined(ENABLE_FINSH) */
