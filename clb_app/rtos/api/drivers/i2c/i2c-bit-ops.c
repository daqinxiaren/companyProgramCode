                                ...
                                struct rt_i2c_msg        *msg)
{
    ...
    rt_uint16_t ignore_nack = msg->flags & DEV_I2C_IGNORE_NACK;
                                ...
                                struct rt_i2c_msg        *msg)
{
    ...
    while (count > 0)
    {
        ...
        LOG_D("recieve bytes: 0x%02x, %s",
                val, (flags & DEV_I2C_NO_READ_ACK) ?
                "(No ACK/NACK)" : (count ? "ACK" : "NACK"));

        if (!(flags & DEV_I2C_NO_READ_ACK))
        {
                              ...
                              rt_uint32_t               num)
{
    ...
    {
        msg = &msgs[i];
        ignore_nack = msg->flags & DEV_I2C_IGNORE_NACK;
        if (!(msg->flags & DEV_I2C_NO_START))
        {
            if (i)
            {
                i2c_restart(ops);
