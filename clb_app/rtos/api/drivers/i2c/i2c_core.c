                                    ...
                                    const char               *bus_name)
{
    ...
    rt_mutex_init(&bus->lock, "i2c_bus_lock", IPC_FLAG_FIFO);

    if (bus->timeout == 0) bus->timeout = TICK_PER_SECOND;
...
struct rt_i2c_bus_device *rt_i2c_bus_device_find(const char *bus_name)
{
    ...
    if (dev == OS_NULL || dev->type != DEVICE_CLASS_I2C_BUS)
    {
    ...
    if (bus->ops->master_xfer)
    {
        ...
        for (ret = 0; ret < num; ret++)
        {
            LOG_D("msgs[%d] %c, addr=0x%02x, len=%d", ret,
                  (msgs[ret].flags & DEV_I2C_RD) ? 'R' : 'W',
                  msgs[ret].addr, msgs[ret].len);
        }
#endif

        rt_mutex_take(&bus->lock, WAITING_FOREVER);
