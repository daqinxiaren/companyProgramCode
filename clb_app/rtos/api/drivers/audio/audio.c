...
static rt_err_t _audio_send_replay_frame(struct rt_audio_device *audio)
{
    ...
    if (rt_data_queue_peek(&audio->replay->queue, (const void **)&data, &src_size) != OS_EOK)
    {
        /* ack stop event */
    ...
    else
    {
        ...
        while (index < dst_size)
        {
            ...
            if (audio->replay->read_index == src_size)
            {
                /* free memory */
                audio->replay->read_index = 0;
                rt_data_queue_pop(&audio->replay->queue, (const void **)&data, &src_size, WAITING_NO);
                rt_mp_free(data);
...
static rt_err_t _aduio_replay_stop(struct rt_audio_device *audio)
{
    ...
    if (audio->replay->activated == OS_TRUE)
    {
        ...
        rt_completion_wait(&audio->replay->cmp, WAITING_FOREVER);
        audio->replay->event &= ~REPLAY_EVT_STOP;
...
static rt_err_t _audio_record_start(struct rt_audio_device *audio)
{
    ...
    if (audio->record->activated != OS_TRUE)
    {
        ...
        rt_device_open(RT_DEVICE(&audio->record->pipe), DEVICE_OFLAG_RDONLY);
...
static rt_err_t _audio_dev_init(struct rt_device *dev)
{
    ...
    /* initialize replay */
    if (dev->flag & DEVICE_FLAG_WRONLY)
    {
        ...
        replay->mp = rt_mp_create("adu_mp", AUDIO_REPLAY_MP_BLOCK_COUNT, AUDIO_REPLAY_MP_BLOCK_SIZE);
        ...
        rt_data_queue_init(&replay->queue, CFG_AUDIO_REPLAY_QUEUE_COUNT, 0, OS_NULL);

        /* init mutex lock for audio replay */
        rt_mutex_init(&replay->lock, "replay", IPC_FLAG_PRIO);
    ...
    /* initialize record */
    if (dev->flag & DEVICE_FLAG_RDONLY)
    {
        ...
        buffer = rt_malloc(AUDIO_RECORD_PIPE_SIZE);
        if (buffer == OS_NULL)
        {
            rt_free(record);
        ...
        rt_audio_pipe_init(&record->pipe, "record",
                           (rt_int32_t)(PIPE_FLAG_FORCE_WR | PIPE_FLAG_BLOCK_RD),
                           buffer,
                           AUDIO_RECORD_PIPE_SIZE);
...
static rt_err_t _audio_dev_open(struct rt_device *dev, rt_uint16_t oflag)
{
    ...
    if ((oflag & DEVICE_OFLAG_RDONLY) && !(dev->flag & DEVICE_FLAG_RDONLY))
        return -OS_EIO;
    if ((oflag & DEVICE_OFLAG_WRONLY) && !(dev->flag & DEVICE_FLAG_WRONLY))
        return -OS_EIO;
    ...
    if (oflag & DEVICE_OFLAG_WRONLY)
    {
        ...
        dev->open_flag |= DEVICE_OFLAG_WRONLY;
    }

    if (oflag & DEVICE_OFLAG_RDONLY)
    {
        /* open record pipe */
        if (audio->record->activated != OS_TRUE)
        {
        ...
        dev->open_flag |= DEVICE_OFLAG_RDONLY;
    }
...
static rt_err_t _audio_dev_close(struct rt_device *dev)
{
    ...
    if (dev->open_flag & DEVICE_OFLAG_WRONLY)
    {
        /* stop replay stream */
    ...
    if (dev->open_flag & DEVICE_OFLAG_RDONLY)
    {
        /* stop record stream */
...
static rt_size_t _audio_dev_read(struct rt_device *dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    ...
    if (!(dev->open_flag & DEVICE_OFLAG_RDONLY) || (audio->record == OS_NULL))
        return 0;
...
static rt_size_t _audio_dev_write(struct rt_device *dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
    ...
    if (!(dev->open_flag & DEVICE_OFLAG_WRONLY) || (audio->replay == OS_NULL))
        return 0;
    ...
    rt_mutex_take(&audio->replay->lock, WAITING_FOREVER);
    while (index < size)
    {
        /* request buffer from replay memory pool */
        if (audio->replay->write_index % block_size == 0)
        {
            audio->replay->write_data = rt_mp_alloc(audio->replay->mp, WAITING_FOREVER);
...
rt_err_t rt_audio_register(struct rt_audio_device *audio, const char *name, rt_uint32_t flag, void *data)
{
    ...
    device->type = DEVICE_CLASS_SOUND;
    ...
    result = rt_device_register(device, name, flag | DEVICE_FLAG_REMOVABLE);
