...
#if defined(WLAN_MANAGE_ENABLE) && defined(WLAN_MSH_CMD_ENABLE)
...
static int wifi_scan(int argc, char *argv[])
{
    struct rt_wlan_scan_result *scan_result = OS_NULL;
    struct rt_wlan_info *info = OS_NULL;
...
static int wifi_debug_save_cfg(int argc, char *argv[])
{
    struct rt_wlan_cfg_info cfg_info;
    int len;
    char *ssid = OS_NULL, *password = OS_NULL;
    ...
    if (password != OS_NULL)
    {
        ...
        if (len > WLAN_PASSWORD_MAX_LENGTH)
        {
...
static int wifi_debug_set_autoconnect(int argc, char *argv[])
{
    if (argc == 2)
    {
        if (rt_strcmp(argv[1], "enable") == 0)
            rt_wlan_config_autoreconnect(OS_TRUE);
        else if (rt_strcmp(argv[1], "disable") == 0)
            rt_wlan_config_autoreconnect(OS_FALSE);
    }
    else
    {
        return -1;
