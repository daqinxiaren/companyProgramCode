...
#if defined(WLAN_PROT_ENABLE) && defined(WLAN_PROT_LWIP_ENABLE)

#ifdef ENABLE_LWIP
#include <netif/ethernetif.h>
#include <lwip/netifapi.h>
#ifdef LWIP_ENABLE_DHCPD
...
static void rt_wlan_lwip_event_handle(struct rt_wlan_prot *port, struct rt_wlan_device *wlan, int event)
{
    ...
    switch (event)
    {
    case WLAN_PROT_EVT_CONNECT:
    {
        LOG_D("event: CONNECT");
        lwip_prot->connected_flag = OS_TRUE;
        break;
    }
    case WLAN_PROT_EVT_DISCONNECT:
    {
        LOG_D("event: DISCONNECT");
        lwip_prot->connected_flag = OS_FALSE;
        break;
    }
    case WLAN_PROT_EVT_AP_START:
    {
        LOG_D("event: AP_START");
        lwip_prot->connected_flag = OS_TRUE;
        break;
    }
    case WLAN_PROT_EVT_AP_STOP:
    {
        LOG_D("event: AP_STOP");
        lwip_prot->connected_flag = OS_FALSE;
        break;
    }
    case WLAN_PROT_EVT_AP_ASSOCIATED:
    {
        LOG_D("event: ASSOCIATED");
        break;
    }
    case WLAN_PROT_EVT_AP_DISASSOCIATED:
    {
...
static rt_err_t rt_wlan_lwip_protocol_control(rt_device_t device, int cmd, void *args)
{
    ...
    case NIOCTL_GADDR:
        ...
        err = rt_device_control((rt_device_t)wlan, WLAN_CMD_GET_MAC, args);
        break;
    default :
        break;
    }
...
static struct rt_wlan_prot *rt_wlan_lwip_protocol_register(struct rt_wlan_prot *prot, struct rt_wlan_device *wlan)
{
    ...
    if (rt_device_open((rt_device_t)wlan, DEVICE_OFLAG_RDWR) != OS_EOK)
    {
...
int rt_wlan_lwip_init(void)
{
    ...
    rt_strncpy(&prot.name[0], WLAN_PROT_LWIP_NAME, WLAN_PROT_NAME_LEN);
    prot.ops = &ops;

    if (rt_wlan_prot_regisetr(&prot) != OS_EOK)
    {
        LOG_E("F:%s L:%d protocol regisetr failed", __FUNCTION__, __LINE__);
        return -1;
    }

    for (event = WLAN_PROT_EVT_INIT_DONE; event < WLAN_PROT_EVT_MAX; event++)
    {
