...
#endif /* WLAN_CFG_DEBUG */
#include <rtdbg.h>

#ifdef WLAN_CFG_ENABLE

#define WLAN_CFG_LOCK()      (rt_mutex_take(&cfg_mutex, WAITING_FOREVER))
#define WLAN_CFG_UNLOCK()    (rt_mutex_release(&cfg_mutex))

#if WLAN_CFG_INFO_MAX < 1
...
void rt_wlan_cfg_init(void)
{
    ...
    if (cfg_cache == OS_NULL)
    {
        ...
        /* init mutex lock */
        rt_mutex_init(&cfg_mutex, "wlan_cfg", IPC_FLAG_FIFO);
    }
}
...
rt_err_t rt_wlan_cfg_cache_refresh(void)
{
    ...
    /* cache is full! exit */
    if (cfg_cache == OS_NULL || cfg_cache->num >= WLAN_CFG_INFO_MAX)
        return -OS_ERROR;

    /* check callback */
    if ((cfg_ops == OS_NULL) ||
            (cfg_ops->get_len == OS_NULL) ||
            (cfg_ops->read_cfg == OS_NULL))
        return -OS_ERROR;
    ...
    LOG_D("head->magic:0x%08x  WLAN_CFG_MAGIC:0x%08x", head->magic, WLAN_CFG_MAGIC);
    ...
    for (i = 0; i < head->num; i++)
    {
        ...
        for (j = 0; j < cfg_cache->num; j++)
        {
            if ((cfg_cache->cfg_info[j].info.ssid.len == cfg_info[i].info.ssid.len) &&
                    ...
                    (rt_memcmp(&cfg_cache->cfg_info[j].info.bssid[0], &cfg_info[i].info.bssid[0], WLAN_BSSID_MAX_LENGTH) == 0))
            {
                equal_flag = OS_TRUE;
                break;
            }
        ...
        if (cfg_cache->num >= WLAN_CFG_INFO_MAX)
        {
            break;
        }
...
rt_err_t rt_wlan_cfg_save(struct rt_wlan_cfg_info *cfg_info)
{
    ...
    for (i = 0; i < cfg_cache->num; i++)
    {
        if ((cfg_cache->cfg_info[i].info.ssid.len == cfg_info->info.ssid.len) &&
                ...
                (rt_memcmp(&cfg_cache->cfg_info[i].info.bssid[0], &cfg_info->info.bssid[0], WLAN_BSSID_MAX_LENGTH) == 0))
        {
            idx = i;
            break;
        }
    ...
    if ((idx == -1) && (cfg_cache->num < WLAN_CFG_INFO_MAX))
    {
        t_info = rt_realloc(cfg_cache->cfg_info, sizeof(struct rt_wlan_cfg_info) * (cfg_cache->num + 1));
        if (t_info == OS_NULL)
        {
    ...
    /* move cache info */
    i = (i >= WLAN_CFG_INFO_MAX ? WLAN_CFG_INFO_MAX - 1 : i);
    for (; i; i--)
    {
