 ...
 *
 * @return rt_err_t if set success, return OS_EOK.
 *
 */
rt_err_t set_date(rt_uint32_t year, rt_uint32_t month, rt_uint32_t day)
{
    ...
    ret = rt_device_control(device, DEVICE_CTRL_RTC_SET_TIME, &now);
...
static void ntp_sync_thread_enrty(void *param)
{
    ...
    rt_thread_delay(RTC_NTP_FIRST_SYNC_DELAY * TICK_PER_SECOND);

    while (1)
    {
        ntp_sync_to_rtc(NULL);
        rt_thread_delay(RTC_NTP_SYNC_PERIOD * TICK_PER_SECOND);
    }
}
...
int rt_rtc_ntp_sync_init(void)
{
    ...
    thread = rt_thread_create("ntp_sync", ntp_sync_thread_enrty, OS_NULL, 1536, 26, 2);
    if (thread)
    {
