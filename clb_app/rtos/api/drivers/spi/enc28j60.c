...
#define enc28j60_lock(dev)      rt_mutex_take(&((struct net_device*)dev)->lock, WAITING_FOREVER);
...
static rt_err_t enc28j60_init(rt_device_t dev)
{
    ...
    rt_thread_delay(TICK_PER_SECOND / 50); /* delay 20ms */
...
static rt_err_t enc28j60_tx(rt_device_t dev, struct pbuf *p)
{
    ...
    if (tx_current->free == OS_FALSE)
    {
        NET_DEBUG("[Tx] no empty buffer!\r\n");
        while (tx_current->free == OS_FALSE)
        {
            rt_err_t result;
            rt_uint32_t recved;
            ...
            result = rt_event_recv(&tx_event, 0x01,
                                   EVENT_FLAG_AND | EVENT_FLAG_CLEAR, WAITING_FOREVER, &recved);
...
rt_err_t enc28j60_attach(const char *spi_device_name)
{
    ...
    /* config spi */
    {
        ...
        cfg.data_width = 8;
        cfg.mode = DEV_SPI_MODE_0 | DEV_SPI_MSB; /* SPI Compatible Modes 0 */
    ...
    rt_event_init(&tx_event, "eth_tx", IPC_FLAG_FIFO);
    ...
    enc28j60_dev.parent.parent.type    = DEVICE_CLASS_NETIF;
#ifdef ENABLE_DEVICE_OPS
    ...
    rt_mutex_init(&enc28j60_dev.lock, "enc28j60", IPC_FLAG_FIFO);
