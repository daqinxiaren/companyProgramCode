...
{
    struct rw009_resp *resp_return = OS_NULL;
    ...
    case RW009_CMD_EASY_JOIN:
        ...
        if( resp->result == 0 )
        {
            ...
            eth_device_linkchange(&wifi_device->parent, OS_TRUE);
        }
        else
        {
            wifi_device->active = 1;
            eth_device_linkchange(&wifi_device->parent, OS_FALSE);
...
static rt_err_t rw009_cmd(struct rw009_wifi *wifi_device, uint32_t cmd, void *args)
{
    ...
    struct rw009_cmd *wifi_cmd = OS_NULL;
    struct rw009_resp *resp = OS_NULL;

    wifi_device->last_cmd = cmd;

    data_packet = (struct spi_data_packet *)rt_mp_alloc(&wifi_device->spi_tx_mp, WAITING_FOREVER);
    ...
    {
        wifi_cmd->len = 0;
        timeout += TICK_PER_SECOND*10;

        if(wifi_device->ap_scan)
        {
            rt_free(wifi_device->ap_scan);
            wifi_device->ap_scan = OS_NULL;
...
static rt_err_t spi_wifi_transfer(struct rw009_wifi *dev)
{
    ...
    rt_err_t result;
    const struct spi_data_packet *data_packet = OS_NULL;
    ...
    result = rt_mb_recv(&wifi_device->spi_tx_mb,
                        ...
                        0);
    if ((result == OS_EOK) && (data_packet != OS_NULL) && (data_packet->data_len > 0))
    {
...
struct pbuf *rw009_wifi_rx(rt_device_t dev)
{
    ...
    if (rt_mb_recv(&wifi_device->eth_rx_mb, (rt_uint32_t *)&p, 0) != OS_EOK)
    {
        return OS_NULL;
    }
...
static void spi_wifi_data_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        if (rt_event_recv(&spi_wifi_data_event,
                          1,
                          EVENT_FLAG_AND | EVENT_FLAG_CLEAR,
                          WAITING_FOREVER,
                          &e) != OS_EOK)
        {
            continue;
...
rt_err_t rt_hw_wifi_init(const char *spi_device_name, wifi_mode_t mode)
{
    ...
    OS_ASSERT( (SPI_MAX_DATA_LEN & 0x03) == 0);
    OS_ASSERT( sizeof(struct rw009_resp) <= SPI_MAX_DATA_LEN);
    ...
    if (rw009_wifi_device.rt_spi_device == OS_NULL)
    {
    ...
    /* config spi */
    {
        ...
        cfg.data_width = 8;
        cfg.mode = DEV_SPI_MODE_0 | DEV_SPI_MSB; /* SPI Compatible: Mode 0. */
...
#endif
    rw009_wifi_device.parent.parent.user_data  = OS_NULL;
    ...
    rt_mb_init(&rw009_wifi_device.rw009_cmd_mb,
               ...
               IPC_FLAG_PRIO);
    rt_event_init(&spi_wifi_data_event, "wifi", IPC_FLAG_FIFO);
    ...
    eth_device_linkchange(&rw009_wifi_device.parent, OS_FALSE);
