...
#ifndef SFUD_DEFAULT_SPI_CFG

#ifndef SFUD_SPI_MAX_HZ
...
{                                                \
    .mode = DEV_SPI_MODE_0 | DEV_SPI_MSB,          \
    .data_width = 8,                             \
    .max_hz = SFUD_SPI_MAX_HZ,                \
}
#endif /* SFUD_DEFAULT_SPI_CFG */

#ifdef SFUD_ENABLE_QSPI
#define SFUD_DEFAULT_QSPI_CFG                 \
{                                                \
    SFUD_DEFAULT_SPI_CFG,                     \
    .medium_size = 0x800000,                     \
    .ddr_mode = 0,                               \
    .qspi_dl_width = 4,                          \
}
#endif /* SFUD_ENABLE_QSPI */
...
static rt_err_t rt_sfud_control(rt_device_t dev, int cmd, void *args) {
    ...
    case DEVICE_CTRL_BLK_GETGEOME: {
        ...
        if (rtt_dev == OS_NULL || geometry == OS_NULL) {
            return -OS_ERROR;
        }
    ...
    case DEVICE_CTRL_BLK_ERASE: {
        ...
        if (addrs == OS_NULL || start_addr > end_addr || rtt_dev == OS_NULL || sfud_dev == OS_NULL) {
            return -OS_ERROR;
        }
...
static sfud_err spi_write_read(const sfud_spi *spi, const uint8_t *write_buf, size_t write_size, uint8_t *read_buf,
    ...
    OS_ASSERT(rtt_dev);
#ifdef SFUD_ENABLE_QSPI
    struct rt_qspi_device *qspi_dev = OS_NULL;
#endif
    if (write_size) {
        OS_ASSERT(write_buf);
    }
...
#ifdef SFUD_ENABLE_QSPI
    if(rtt_dev->rt_spi_device->bus->mode & DEV_SPI_BUS_MODE_QSPI) {
    ...
    {
        if (write_size && read_size) {
            if (rt_spi_send_then_recv(rtt_dev->rt_spi_device, write_buf, write_size, read_buf, read_size) != OS_EOK) {
                result = SFUD_ERR_TIMEOUT;
            }
        } else if (write_size) {
...
static void spi_lock(const sfud_spi *spi) {
    ...
    rt_mutex_take(&(rtt_dev->lock), WAITING_FOREVER);
}
...
static void retry_delay_100us(void) {
    ...
    rt_thread_delay((TICK_PER_SECOND * 1 + 9999) / 10000);
}
...
sfud_err sfud_spi_port_init(sfud_flash *flash) {
    ...
    if (TICK_PER_SECOND < 1000) {
        LOG_W("[SFUD] Warning: The OS tick(%d) is less than 1000. So the flash write will take more time.", TICK_PER_SECOND);
    }
    /* 100 microsecond delay */
 ...
 *
 * @return probed SPI flash device, probe failed will return OS_NULL
 */
...
{
    rt_spi_flash_device_t rtt_dev = OS_NULL;
    ...
    if (rtt_dev) {
        ...
        /* initialize lock */
        rt_mutex_init(&(rtt_dev->lock), spi_flash_dev_name, IPC_FLAG_FIFO);
    }
    ...
    if (rtt_dev && sfud_dev && spi_flash_dev_name_bak && spi_dev_name_bak) {
        ...
        /* SPI configure */
        {
            ...
            if (rtt_dev->rt_spi_device == OS_NULL || rtt_dev->rt_spi_device->parent.type != DEVICE_CLASS_SPI_DEVICE) {
        ...
        /* register device */
        rtt_dev->flash_device.type = DEVICE_CLASS_BLOCK;
#ifdef ENABLE_DEVICE_OPS
        rtt_dev->flash_device.ops  = &flash_device_ops;
#else
        rtt_dev->flash_device.init = OS_NULL;
        rtt_dev->flash_device.open = OS_NULL;
        rtt_dev->flash_device.close = OS_NULL;
        ...
        rt_device_register(&(rtt_dev->flash_device), spi_flash_dev_name, DEVICE_FLAG_RDWR | DEVICE_FLAG_STANDALONE);
...
{
    struct rt_spi_configuration cfg = SFUD_DEFAULT_SPI_CFG;
#ifndef SFUD_ENABLE_QSPI
    return rt_sfud_flash_probe_ex(spi_flash_dev_name, spi_dev_name, &cfg, OS_NULL);
#else
    struct rt_qspi_configuration qspi_cfg = SFUD_DEFAULT_QSPI_CFG;
...
{
    rt_spi_flash_device_t  rtt_dev       = OS_NULL;
    struct rt_spi_device  *rt_spi_device = OS_NULL;
    sfud_flash_t           sfud_dev      = OS_NULL;

    rt_spi_device = (struct rt_spi_device *) rt_device_find(spi_dev_name);
    if (rt_spi_device == OS_NULL || rt_spi_device->parent.type != DEVICE_CLASS_SPI_DEVICE) {
        LOG_E("ERROR: SPI device %s not found!", spi_dev_name);
        goto __error;
    }
...
sfud_flash_t rt_sfud_flash_find_by_dev_name(const char *flash_dev_name)
{
    ...
    if (rtt_dev == OS_NULL || rtt_dev->flash_device.type != DEVICE_CLASS_BLOCK) {
...
#if defined(ENABLE_FINSH) && defined(FINSH_ENABLE_MSH)
...
#endif /* defined(ENABLE_FINSH) && defined(FINSH_ENABLE_MSH) */
