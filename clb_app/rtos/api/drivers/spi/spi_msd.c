...
static rt_err_t MSD_take_owner(struct rt_spi_device *spi_device)
{
    ...
    result = rt_mutex_take(&(spi_device->bus->lock), WAITING_FOREVER);
    if (result == OS_EOK)
    {
...
//                if((result != OS_EOK) || (response[0] == 0x01))
                if (result != OS_EOK)
                {
                    rt_spi_release(msd->spi_device);
                    continue;
...
rt_err_t msd_init(const char *sd_device_name, const char *spi_device_name)
{
    ...
    _msd_device.parent.type    = DEVICE_CLASS_BLOCK;
