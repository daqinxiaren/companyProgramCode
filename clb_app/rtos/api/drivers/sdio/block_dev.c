...
static rt_list_t blk_devices = OS_LIST_OBJECT_INIT(blk_devices);
                               ...
                               rt_size_t   size)
{
    ...
    rt_sem_take(part->lock, WAITING_FOREVER);
    while (remain_size)
    {
...
rt_int32_t rt_mmcsd_blk_probe(struct rt_mmcsd_card *card)
{
    ...
    if (status == OS_EOK)
    {
        for (i = 0; i < MMCSD_MAX_PARTITION; i++)
        {
            ...
            if (status == OS_EOK)
            {
                ...
                blk_dev->part.lock = rt_sem_create(sname, 1, IPC_FLAG_FIFO);
    
                /* register mmcsd device */
                blk_dev->dev.type = DEVICE_CLASS_BLOCK;
#ifdef ENABLE_DEVICE_OPS
                ...
                rt_device_register(&blk_dev->dev, dname,
                    DEVICE_FLAG_RDWR | DEVICE_FLAG_REMOVABLE | DEVICE_FLAG_STANDALONE);
            ...
            else
            {
                if (i == 0)
                {
                    ...
                    blk_dev->part.size   = 0;
                    blk_dev->part.lock = rt_sem_create("sem_sd0", 1, IPC_FLAG_FIFO);
