...
{
    rt_mutex_take(&host->bus_lock, WAITING_FOREVER);
}
...
void mmcsd_send_request(struct rt_mmcsd_host *host, struct rt_mmcsd_req *req)
{
    do {
        ...
        rt_sem_take(&host->sem_ack, WAITING_FOREVER);
...
int mmcsd_wait_cd_changed(rt_int32_t timeout)
{
    ...
    if (rt_mb_recv(&mmcsd_hotpluge_mb, (rt_ubase_t *)&host, timeout) == OS_EOK)
    {
...
OS_MODE_EXPORT(mmcsd_wait_cd_changed);
...
void mmcsd_detect(void *param)
{
    ...
    while (1) 
    {
        if (rt_mb_recv(&mmcsd_detect_mb, (rt_ubase_t *)&host, WAITING_FOREVER) == OS_EOK)
        {
            if (host->card == OS_NULL)
            {
...
struct rt_mmcsd_host *mmcsd_alloc_host(void)
{
    ...
    rt_mutex_init(&host->bus_lock, "sd_bus_lock", IPC_FLAG_FIFO);
    rt_sem_init(&host->sem_ack, "sd_ack", 0, IPC_FLAG_FIFO);
...
int rt_mmcsd_core_init(void)
{
   ...
   ret = rt_mb_init(&mmcsd_hotpluge_mb, "mmcsdhotplugmb",
        ...
        IPC_FLAG_FIFO);
    OS_ASSERT(ret == OS_EOK);
     ret = rt_thread_init(&mmcsd_detect_thread, "mmcsd_detect", mmcsd_detect, OS_NULL, 
                 &mmcsd_stack[0], MMCSD_STACK_SIZE, MMCSD_THREAD_PREORITY, 20);
    if (ret == OS_EOK) 
    {
