...
#ifndef SDIO_STACK_SIZE
#define SDIO_STACK_SIZE 512
#endif
#ifndef SDIO_THREAD_PRIORITY
#define SDIO_THREAD_PRIORITY  0x40
#endif

static rt_list_t sdio_cards = OS_LIST_OBJECT_INIT(sdio_cards);
static rt_list_t sdio_drivers = OS_LIST_OBJECT_INIT(sdio_drivers);
...
static void sdio_irq_thread(void *param)
{
    ...
    while (1) 
    {
        if (rt_sem_take(host->sdio_irq_sem, WAITING_FOREVER) == OS_EOK)
        {
...
rt_int32_t sdio_enable_func(struct rt_sdio_function *func)
{
    ...
    timeout = rt_tick_get() + func->enable_timeout_val * TICK_PER_SECOND / 1000;
