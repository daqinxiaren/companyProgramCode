...
一 CAN Driver 注册
	...
	mode ：
		#define DEV_CAN_MODE_NORMAL 0		正常模式
		#define DEV_CAN_MODE_LISEN 1		只听模式
		#define DEV_CAN_MODE_LOOPBACK 2		自发自收模式
		#define DEV_CAN_MODE_LOOPBACKANLISEN 3	自发自收只听模式
		配置Can 的工作状态。
	privmode :
		#define DEV_CAN_MODE_PRIV 0x01 		处于优先级模式，高优先级的消息优先发送。
	...
	开启RT_CAN_ENABLE_LED时要确保当前系统已实现pin驱动。
	rt_uint32_t ticks : 配置Can driver timer周期。
	#ifdef DEV_CAN_ENABLE_HDR
		rt_uint32_t maxhdr;
	#endif
	如果使用硬件过滤，则开启RT_CAN_ENABLE_HDR, maxhdr 为Can控制器过滤表的数量。
	2、struct rt_can_ops
	{
...
二、 CAN Driver 的添加：
	...
	3、can口中断，要完接收，发送结束，以及错误中断。
		...
		#define DEV_CAN_EVENT_TX_FAIL         0x03    /* Tx complete   */
