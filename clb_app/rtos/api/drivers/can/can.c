 ...
 * 2015-07-06     Bernard           code cleanup and remove DEV_CAN_ENABLE_LED;
 */
...
#define CAN_LOCK(can)   rt_mutex_take(&(can->lock), WAITING_FOREVER)
...
rt_inline int _can_int_rx(struct rt_can_device *can, struct rt_can_msg *data, int msgs)
{
	...
	while (msgs)
	{
...
#ifdef DEV_CAN_ENABLE_HDR
        rt_int8_t hdr;
#endif /*DEV_CAN_ENABLE_HDR*/
		struct rt_can_msg_list *listmsg = OS_NULL;
...
#ifdef DEV_CAN_ENABLE_HDR
                rt_list_remove(&listmsg->hdrlist);
                if (listmsg->owner != OS_NULL && listmsg->owner->msgs)
                {
                    listmsg->owner->msgs--;
                }
                listmsg->owner = OS_NULL;
		...
		if (listmsg != OS_NULL)
		{
			...
			rx_fifo->freenumbers++;
			OS_ASSERT(rx_fifo->freenumbers <= can->config.msgboxsz);
			rt_hw_interrupt_enable(level);
...
rt_inline int _can_int_tx(struct rt_can_device *can, const struct rt_can_msg *data, int msgs)
{
	...
	while (msgs)
	{
		...
		rt_uint32_t no;
		rt_uint32_t result;
		struct rt_can_sndbxinx_list *tx_tosnd = OS_NULL;
		
		rt_sem_take(&(tx_fifo->sem), WAITING_FOREVER);
		level = rt_hw_interrupt_disable();
		...
		tx_tosnd->result = DEV_CAN_SND_RESULT_WAIT;
		if (can->ops->sendmsg(can, data, no) != OS_EOK)
		{
			/* send failed. */
			level = rt_hw_interrupt_disable();
		...
		rt_completion_wait(&(tx_tosnd->completion), WAITING_FOREVER);
		...
		if (result == DEV_CAN_SND_RESULT_OK)
		{
			level = rt_hw_interrupt_disable();
			can->status.sndpkg++;
			rt_hw_interrupt_enable(level);
...
rt_inline int _can_int_tx_priv(struct rt_can_device *can, const struct rt_can_msg *data, int msgs)
{
	...
	while (msgs)
	{
		...
		level = rt_hw_interrupt_disable();
		if ((tx_fifo->buffer [no].result != DEV_CAN_SND_RESULT_OK))
		{
			rt_hw_interrupt_enable(level);
			
			rt_completion_wait(&(tx_fifo->buffer [no].completion), WAITING_FOREVER);
			continue;
		}
		tx_fifo->buffer [no].result = DEV_CAN_SND_RESULT_WAIT;
		rt_hw_interrupt_enable(level);
...
static rt_err_t rt_can_open(struct rt_device *dev, rt_uint16_t oflag)
{
	...
	if (can->can_rx == OS_NULL)
	{
		if (oflag & DEVICE_FLAG_INT_RX)
		{
			...
			rx_fifo = (struct rt_can_rx_fifo*) rt_malloc(
			        sizeof(struct rt_can_rx_fifo) + can->config.msgboxsz * sizeof(struct rt_can_msg_list));
			OS_ASSERT(rx_fifo != OS_NULL);
			...
			for (i = 0; i < can->config.msgboxsz; i++)
			{
                ...
                rx_fifo->buffer[i].owner = OS_NULL;
#endif
			}
			can->can_rx = rx_fifo;
	...
	if (can->can_tx == OS_NULL)
	{
		if (oflag & DEVICE_FLAG_INT_TX)
		{
			...
			tx_fifo = (struct rt_can_tx_fifo*) rt_malloc(
			        sizeof(struct rt_can_tx_fifo) + can->config.sndboxnumber * sizeof(struct rt_can_sndbxinx_list));
			OS_ASSERT(tx_fifo != OS_NULL);
			
			tx_fifo->buffer = (struct rt_can_sndbxinx_list*) (tx_fifo + 1);
			rt_memset(tx_fifo->buffer, 0, can->config.sndboxnumber * sizeof(struct rt_can_sndbxinx_list));
			...
			for (i = 0; i < can->config.sndboxnumber; i++)
			{
				...
				tx_fifo->buffer[i].result = DEV_CAN_SND_RESULT_OK;
			}
			
			rt_sprintf(tmpname, "%stl", dev->parent.name);
			rt_sem_init(&(tx_fifo->sem), tmpname, can->config.sndboxnumber, IPC_FLAG_FIFO);
			can->can_tx = tx_fifo;
...
static rt_size_t rt_can_read(struct rt_device *dev, rt_off_t pos, void *buffer, rt_size_t size)
{
	struct rt_can_device *can;
...
static rt_size_t rt_can_write(struct rt_device *dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
	struct rt_can_device *can;
...
static rt_err_t rt_can_control(struct rt_device *dev, int cmd, void *args)
{
		...
		case DEV_CAN_CMD_SET_PRIV:
			...
			if ((rt_uint32_t) args != can->config.privmode)
			{
				...
				else
				{
					...
					{
						level = rt_hw_interrupt_disable();
						if (tx_fifo->buffer [i].result == DEV_CAN_SND_RESULT_OK)
						{
							rt_list_insert_before(&tx_fifo->freelist, &tx_fifo->buffer [i].list);
						}
						rt_hw_interrupt_enable(level);
    ...
    case DEV_CAN_CMD_SET_FILTER:
        ...
        break;
#endif /*DEV_CAN_ENABLE_HDR*/
#ifdef DEV_CAN_ENABLE_BUS_HOOK
    case DEV_CAN_CMD_SET_BUS_HOOK:
        can->bus_hook = (rt_can_bus_hook) args;
        break;
#endif /*DEV_CAN_ENABLE_BUS_HOOK*/
		default:
			/* control device */
...
static void cantimeout(void *arg)
{
	...
	can = (rt_can_t) arg;
	OS_ASSERT(can);
	rt_device_control((rt_device_t) can, DEV_CAN_CMD_GET_STATUS, (void*) &can->status);
	
	if (can->status_indicate.ind != OS_NULL)
	{
...
rt_err_t rt_hw_can_register(struct rt_can_device *can, const char *name, const struct rt_can_ops *ops, void *data)
{
	...
	device->type = DEVICE_CLASS_CAN_BUS;
	...
	can->can_rx = OS_NULL;
	can->can_tx = OS_NULL;
	rt_mutex_init(&(can->lock), "can", IPC_FLAG_PRIO);
#ifdef DEV_CAN_ENABLE_BUS_HOOK
	...
	can->timerinitflag = 0;
	rt_timer_init(&can->timer, name, cantimeout, (void*) can, can->config.ticks,
	TIMER_FLAG_PERIODIC);
...
void rt_hw_can_isr(struct rt_can_device *can, int event)
{
	switch (event & 0xff)
	{
        ...
        rt_int8_t hdr;
			...
			OS_ASSERT(can->parent.open_flag & DEVICE_FLAG_INT_RX);
            ...
            listmsg->owner = OS_NULL;
#endif /*DEV_CAN_ENABLE_HDR*/
				OS_ASSERT(rx_fifo->freenumbers > 0);
				rx_fifo->freenumbers--;
			}
            ...
            if (can->hdr != OS_NULL)
            {
                OS_ASSERT(hdr < can->config.maxhdr && hdr >= 0);
			...
			/* invoke callback */
#ifdef DEV_CAN_ENABLE_HDR
        if (can->hdr != OS_NULL && can->hdr[hdr].connected && can->hdr[hdr].filter.ind)
        {
            rt_size_t rx_length;
            OS_ASSERT(hdr < can->config.maxhdr && hdr >= 0);
		...
		case DEV_CAN_EVENT_TX_DONE:
		case DEV_CAN_EVENT_TX_FAIL:
		{
			struct rt_can_tx_fifo *tx_fifo;
			rt_uint32_t no;
			no = event >> 8;
			...
			if ((event & 0xff) == DEV_CAN_EVENT_TX_DONE)
			{
				tx_fifo->buffer [no].result = DEV_CAN_SND_RESULT_OK;
			}
			else
			{
				tx_fifo->buffer [no].result = DEV_CAN_SND_RESULT_ERR;
			}
...
int cmd_canstat(int argc, void **argv)
{
	...
	if (argc >= 2)
	{
		...
		rt_device_control(candev, DEV_CAN_CMD_GET_STATUS, &status);
		rt_kprintf("\n Receive...error..count: %010ld. Send.....error....count: %010ld.", status.rcverrcnt, status.snderrcnt);
		rt_kprintf("\n Bit..pad..error..count: %010ld. Format...error....count: %010ld", status.bitpaderrcnt,
		        status.formaterrcnt);
		rt_kprintf("\n Ack.......error..count: %010ld. Bit......error....count: %010ld.", status.ackerrcnt, status.biterrcnt);
		rt_kprintf("\n CRC.......error..count: %010ld. Error.code.[%010ld]: ", status.crcerrcnt, status.errcode);
		...
		rt_kprintf("\n Total.receive.packages: %010ld. Droped.receive.packages: %010ld.", status.rcvpkg, status.dropedrcvpkg);
