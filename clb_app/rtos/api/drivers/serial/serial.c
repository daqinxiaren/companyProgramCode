 ...
 * 2013-02-20     bernard      use DEV_SERIAL_RB_BUFSZ to define
 *                             the size of ring buffer.
 * 2014-07-10     bernard      rewrite serial framework
 * 2014-12-31     bernard      use open_flag for poll_tx stream mode.
 * 2015-05-19     Quintin      fix DMA tx mod tx_dma->activated flag !=OS_FALSE BUG
 *                             in open function.
...
static int serial_fops_open(struct dfs_fd *fd)
{
    ...
    case O_RDONLY:
        ...
        flags = DEVICE_FLAG_INT_RX | DEVICE_FLAG_RDONLY;
        break;
    case O_WRONLY:
        LOG_D("fops open: O_WRONLY!");
        flags = DEVICE_FLAG_WRONLY;
        break;
    case O_RDWR:
        LOG_D("fops open: O_RDWR!");
        flags = DEVICE_FLAG_INT_RX | DEVICE_FLAG_RDWR;
        break;
    default:
...
static int serial_fops_close(struct dfs_fd *fd)
{
    ...
    rt_device_set_rx_indicate(device, OS_NULL);
...
static int serial_fops_read(struct dfs_fd *fd, void *buf, size_t count)
{
    ...
    do
    {
        ...
        if (size <= 0)
        {
            ...
            rt_wqueue_wait(&(device->wait_queue), 0, WAITING_FOREVER);
        }
    }while (size <= 0);
...
static int serial_fops_poll(struct dfs_fd *fd, struct rt_pollreq *req)
{
    ...
    if (flags == O_RDONLY || flags == O_RDWR)
    {
        ...
        if ((rx_fifo->get_index != rx_fifo->put_index) || (rx_fifo->get_index == rx_fifo->put_index && rx_fifo->is_full == OS_TRUE))
            mask |= POLLIN;
...
rt_inline int _serial_poll_rx(struct rt_serial_device *serial, rt_uint8_t *data, int length)
{
    ...
    while (length)
    {
        ...
        *data = ch;
        ...
        if(serial->parent.open_flag & DEVICE_FLAG_STREAM)
        {
            if (ch == '\n') break;
        }
    }
...
rt_inline int _serial_int_rx(struct rt_serial_device *serial, rt_uint8_t *data, int length)
{
    ...
    while (length)
    {
        ...
        /* there's no data: */
        if ((rx_fifo->get_index == rx_fifo->put_index) && (rx_fifo->is_full == OS_FALSE))
        {
        ...
        if (rx_fifo->is_full == OS_TRUE)
        {
            rx_fifo->is_full = OS_FALSE;
        }
...
rt_inline int _serial_int_tx(struct rt_serial_device *serial, const rt_uint8_t *data, int length)
{
    ...
    while (length)
    {
        ...
        if (*data == '\n' && (serial->parent.open_flag & DEVICE_FLAG_STREAM))
        {
            ...
            {
                rt_completion_wait(&(tx->completion), WAITING_FOREVER);
                continue;
            }
        }
...
{
    static rt_bool_t already_output = OS_FALSE;

    if (already_output == OS_FALSE)
    {
#if !defined(ENABLE_ULOG) || defined(ULOG_ENABLE_ISR_LOG)
        LOG_W("Warning: There is no enough buffer for saving data,"
              " please increase the DEV_SERIAL_RB_BUFSZ option.");
#endif
        already_output = OS_TRUE;
    }
}

#if defined(ENABLE_POSIX) || defined(DEV_SERIAL_ENABLE_DMA)
static rt_size_t _serial_fifo_calc_recved_len(struct rt_serial_device *serial)
{
    ...
    {
        return (rx_fifo->is_full == OS_FALSE ? 0 : serial->config.bufsz);
    }
    else
    {
        if (rx_fifo->put_index > rx_fifo->get_index)
...
#endif /* ENABLE_POSIX || DEV_SERIAL_ENABLE_DMA */
...
static void rt_dma_recv_update_get_index(struct rt_serial_device *serial, rt_size_t len)
{
    ...
    OS_ASSERT(rx_fifo != OS_NULL);
    OS_ASSERT(len <= rt_dma_calc_recved_len(serial));

    if (rx_fifo->is_full && len != 0) rx_fifo->is_full = OS_FALSE;
...
rt_inline int _serial_dma_rx(struct rt_serial_device *serial, rt_uint8_t *data, int length)
{
    ...
    OS_ASSERT((serial != OS_NULL) && (data != OS_NULL));
    ...
    if (serial->config.bufsz == 0)
    {
        ...
        OS_ASSERT(rx_dma != OS_NULL);

        if (rx_dma->activated != OS_TRUE)
        {
            rx_dma->activated = OS_TRUE;
            OS_ASSERT(serial->ops->dma_transmit != OS_NULL);
            serial->ops->dma_transmit(serial, data, length, DEV_SERIAL_DMA_RX);
        }
        else result = -OS_EBUSY;
        rt_hw_interrupt_enable(level);

        if (result == OS_EOK) return length;
...
rt_inline int _serial_dma_tx(struct rt_serial_device *serial, const rt_uint8_t *data, int length)
{
    ...
    result = rt_data_queue_push(&(tx_dma->data_queue), data, length, WAITING_FOREVER);
    if (result == OS_EOK)
    {
        level = rt_hw_interrupt_disable();
        if (tx_dma->activated != OS_TRUE)
        {
            tx_dma->activated = OS_TRUE;
            rt_hw_interrupt_enable(level);

            /* make a DMA transfer */
            serial->ops->dma_transmit(serial, (rt_uint8_t *)data, length, DEV_SERIAL_DMA_TX);
        }
        else
        {
            rt_hw_interrupt_enable(level);
...
static rt_err_t rt_serial_init(struct rt_device *dev)
{
    ...
    /* initialize rx/tx */
    serial->serial_rx = OS_NULL;
    serial->serial_tx = OS_NULL;
...
static rt_err_t rt_serial_open(struct rt_device *dev, rt_uint16_t oflag)
{
    ...
    /* keep steam flag */
    if ((oflag & DEVICE_FLAG_STREAM) || (dev->open_flag & DEVICE_FLAG_STREAM))
        stream_flag = DEVICE_FLAG_STREAM;

    /* get open flags */
    dev->open_flag = oflag & 0xff;

    /* initialize the Rx/Tx structure according to open flag */
    if (serial->serial_rx == OS_NULL)
    { 
...
#ifdef DEV_SERIAL_ENABLE_DMA
        else if (oflag & DEVICE_FLAG_DMA_TX)
        {
            struct rt_serial_tx_dma* tx_dma;

            tx_dma = (struct rt_serial_tx_dma*) rt_malloc (sizeof(struct rt_serial_tx_dma));
            OS_ASSERT(tx_dma != OS_NULL);
            tx_dma->activated = OS_FALSE;

            rt_data_queue_init(&(tx_dma->data_queue), 8, 4, OS_NULL);
...
static rt_err_t rt_serial_close(struct rt_device *dev)
{
    ...
    if (dev->ref_count > 1) return OS_EOK;
    ...
    else if (dev->open_flag & DEVICE_FLAG_DMA_RX)
    {
        ...
        /* configure low level device */
        serial->ops->control(serial, DEVICE_CTRL_CLR_INT, (void *) DEVICE_FLAG_DMA_RX);
    }
#endif /* DEV_SERIAL_ENABLE_DMA */
...
#ifdef DEV_SERIAL_ENABLE_DMA
    else if (dev->open_flag & DEVICE_FLAG_DMA_TX)
    {
        struct rt_serial_tx_dma* tx_dma;
        ...
        /* configure low level device */
        serial->ops->control(serial, DEVICE_CTRL_CLR_INT, (void *) DEVICE_FLAG_DMA_TX);
    }

    serial->ops->control(serial, DEVICE_CTRL_CLOSE, OS_NULL);
...
static void _tc_flush(struct rt_serial_device *serial, int queue)
{
    rt_base_t level;
    int ch = -1;
    struct rt_serial_rx_fifo *rx_fifo = OS_NULL;
    struct rt_device *device = OS_NULL;
    ...
    switch(queue)
    {
        ...
        case TCIOFLUSH:
            ...
            if((device->open_flag & DEVICE_FLAG_INT_RX) || (device->open_flag & DEVICE_FLAG_DMA_RX))
            {
                OS_ASSERT(OS_NULL != rx_fifo);
                level = rt_hw_interrupt_disable();
                rt_memset(rx_fifo->buffer, 0, serial->config.bufsz);
                rx_fifo->put_index = 0;
                rx_fifo->get_index = 0;
        ...
        case TCGETA:
            {
                ...
                if (tio == OS_NULL) return -OS_EINVAL;
...
void rt_hw_serial_isr(struct rt_serial_device *serial, int event)
{
    switch (event & 0xff)
    {
...
#ifdef DEV_SERIAL_ENABLE_DMA
        case DEV_SERIAL_EVENT_TX_DMADONE:
        {
            const void *data_ptr;
            rt_size_t data_size;
            ...
            if (rt_data_queue_peek(&(tx_dma->data_queue), &data_ptr, &data_size) == OS_EOK)
            {
                /* transmit next data node */
                tx_dma->activated = OS_TRUE;
                serial->ops->dma_transmit(serial, (rt_uint8_t *)data_ptr, data_size, DEV_SERIAL_DMA_TX);
            }
            else
            {
                tx_dma->activated = OS_FALSE;
        ...
        case DEV_SERIAL_EVENT_RX_DMADONE:
        {
            int length;
            rt_base_t level;
            ...
            if (serial->config.bufsz == 0)
            {
                ...
                OS_ASSERT(serial->parent.rx_indicate != OS_NULL);
