...
#if defined(DEV_USBD_MIC_DEVICE_NAME)
    #define MIC_DEVICE_NAME    DEV_USBD_MIC_DEVICE_NAME
#else
...
void mic_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        rt_device_open(mic.dev, DEVICE_OFLAG_RDONLY);
...
{
    rt_thread_t mic_tid;
    mic.event = rt_event_create("mic_event", IPC_FLAG_FIFO);
    ...
    rt_kprintf("\tUSB Audio mic Core init.\r\n");
    return OS_EOK;
}
