...
{
#ifdef DEV_USB_DEVICE_HID_KEYBOARD
    USAGE_PAGE(1),      0x01,
    USAGE(1),           0x06,
    COLLECTION(1),      0x01,
    ...
    INPUT(1),           0x00,
    END_COLLECTION(0),
#if DEV_USB_DEVICE_HID_KEYBOARD_NUMBER>1
    /****keyboard2*****/
    USAGE_PAGE(1),      0x01,
    USAGE(1),           0x06,
    COLLECTION(1),      0x01,
    ...
    INPUT(1),           0x00,
    END_COLLECTION(0),
#if DEV_USB_DEVICE_HID_KEYBOARD_NUMBER>2
    USAGE_PAGE(1),      0x01,
    USAGE(1),           0x06,
    COLLECTION(1),      0x01,
    ...
    INPUT(1),           0x00,
    END_COLLECTION(0),
#if DEV_USB_DEVICE_HID_KEYBOARD_NUMBER>3
    USAGE_PAGE(1),      0x01,
    USAGE(1),           0x06,
    COLLECTION(1),      0x01,
...
#endif
    // Media Control
#ifdef DEV_USB_DEVICE_HID_MEDIA
    USAGE_PAGE(1),      0x0C,
    USAGE(1),           0x01,
    COLLECTION(1),      0x01,
...
#ifdef DEV_USB_DEVICE_HID_GENERAL
    USAGE_PAGE(1),      0x8c,
    USAGE(1),           0x01,
    COLLECTION(1),      0x01,
    REPORT_ID(1),       HID_REPORT_ID_GENERAL,

    REPORT_COUNT(1),    DEV_USB_DEVICE_HID_GENERAL_IN_REPORT_LENGTH,
    USAGE(1),           0x03,
    REPORT_SIZE(1),     0x08,
    LOGICAL_MINIMUM(1), 0x00,
    ...
    REPORT_COUNT(1),    DEV_USB_DEVICE_HID_GENERAL_OUT_REPORT_LENGTH,
    USAGE(1),           0x04,
    REPORT_SIZE(1),     0x08,
    LOGICAL_MINIMUM(1), 0x00,
    LOGICAL_MAXIMUM(1), 0xFF,
    ...
    END_COLLECTION(0),
#endif
#ifdef DEV_USB_DEVICE_HID_MOUSE
...
#if defined(DEV_USB_DEVICE_HID_KEYBOARD)||defined(DEV_USB_DEVICE_HID_MOUSE)
        USB_HID_SUBCLASS_BOOT,    /* bInterfaceSubClass : 1=BOOT, 0=no boot */
#else
        USB_HID_SUBCLASS_NOBOOT,    /* bInterfaceSubClass : 1=BOOT, 0=no boot */
#endif
#if !defined(DEV_USB_DEVICE_HID_KEYBOARD)||!defined(DEV_USB_DEVICE_HID_MOUSE)||!defined(DEV_USB_DEVICE_HID_MEDIA)
        USB_HID_PROTOCOL_NONE,      /* nInterfaceProtocol : 0=none, 1=keyboard, 2=mouse */
#elif !defined(DEV_USB_DEVICE_HID_MOUSE)
...
{
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("_hid_set_report_callback\n"));
...
static rt_err_t _function_enable(ufunction_t func)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("hid function enable\n"));
//
...
static rt_err_t _function_disable(ufunction_t func)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("hid function disable\n"));
...
static void hid_thread_entry(void* parameter)
{
	...
	while(1)
	{
		if(rt_mq_recv(&hiddev->hid_mq, &report, sizeof(report),WAITING_FOREVER) != OS_EOK )
            continue;
...
static void rt_usb_hid_init(struct ufunction *func)
{
    ...
    rt_device_register(&hiddev->parent, "hidd", DEVICE_FLAG_RDWR);
    rt_mq_init(&hiddev->hid_mq, "hiddmq", hid_mq_pool, sizeof(struct hid_report),
                            sizeof(hid_mq_pool), IPC_FLAG_FIFO);
                            
    rt_thread_init(&hid_thread, "hidd", hid_thread_entry, hiddev,
            hid_thread_stack, sizeof(hid_thread_stack), DEV_USBD_THREAD_PRIO, 20);
