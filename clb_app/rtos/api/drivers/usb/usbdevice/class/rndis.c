...
#else
        eth_device_linkchange(&((rt_rndis_eth_t)func->user_data)->parent, OS_TRUE);
#endif /* RNDIS_DELAY_LINK_UP */
        break;
...
static rt_err_t _rndis_reset_response(ufunction_t func,rndis_set_msg_t msg)
{
    ...
    eth_device_linkchange(&((rt_rndis_eth_t)func->user_data)->parent, OS_FALSE);

    /* reset eth rx tx */
    ((rt_rndis_eth_t)func->user_data)->rx_frist = OS_TRUE;
    ((rt_rndis_eth_t)func->user_data)->rx_flag = OS_FALSE;
...
static rt_err_t _rndis_get_encapsulated_response(ufunction_t func, ureq_t setup)
{
    ...
    if(rt_list_isempty(&((rt_rndis_eth_t)func->user_data)->response_list))
    {
        ...
        ((rt_rndis_eth_t)func->user_data)->need_notify = OS_TRUE;
        return OS_EOK;
    }
...
static rt_err_t _ep_out_handler(ufunction_t func, rt_size_t size)
{
    ...
    if(((rt_rndis_eth_t)func->user_data)->rx_frist == OS_TRUE)
    {
        ...
        if (size >= 44)
        {
            ...
            ((rt_rndis_eth_t)func->user_data)->rx_frist = OS_FALSE;
    ...
    {
        ((rt_rndis_eth_t)func->user_data)->rx_frist = OS_TRUE;
        ((rt_rndis_eth_t)func->user_data)->rx_flag = OS_TRUE;
...
rt_err_t rt_rndis_eth_tx(rt_device_t dev, struct pbuf* p)
{
    ...
    //OS_ASSERT(p->tot_len < sizeof(device->tx_buffer));
...
static void timer_timeout(void* parameter)
{
    ...
    eth_device_linkchange(&((rt_rndis_eth_t)parameter)->parent, OS_TRUE);
}
#endif /* RNDIS_DELAY_LINK_UP */
...
ufunction_t rt_usbd_function_rndis_create(udevice_t device)
{
    ...
    rt_sem_init(&_rndis->tx_buffer_free, "ue_tx", 1, IPC_FLAG_FIFO);
