...
static rt_err_t _get_device_descriptor(struct udevice* device, ureq_t setup)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("_get_device_descriptor\n"));
...
static rt_err_t _get_config_descriptor(struct udevice* device, ureq_t setup)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("_get_config_descriptor\n"));
 ...
 * @param setup the setup bRequest.
 *
 * @return OS_EOK on successful, -OS_ERROR on invalid bRequest.
 */
static rt_err_t _get_string_descriptor(struct udevice* device, ureq_t setup)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("_get_string_descriptor\n"));
...
{
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("_get_qualifier_descriptor\n"));
...
static rt_err_t _get_interface(struct udevice* device, ureq_t setup)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("_get_interface\n"));
...
static rt_err_t _set_interface(struct udevice* device, ureq_t setup)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("_set_interface\n"));
...
static rt_err_t _get_config(struct udevice* device, ureq_t setup)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("_get_config\n"));
...
static rt_err_t _set_config(struct udevice* device, ureq_t setup)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("_set_config\n"));
    ...
    if (setup->wValue == 0)
    {
        ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("address state\n"));
...
static rt_err_t _set_address(struct udevice* device, ureq_t setup)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("_set_address\n"));
...
static rt_err_t _request_interface(struct udevice* device, ureq_t setup)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("_request_interface\n"));
...
static rt_err_t _standard_request(struct udevice* device, ureq_t setup)
{
    ...
    case USB_REQ_TYPE_ENDPOINT:
        ...
        case USB_REQ_GET_STATUS:
        {
            ...
            ep = rt_usbd_find_endpoint(device, OS_NULL, setup->wIndex);
            value = ep->stalled;        
            rt_usbd_ep0_write(device, &value, 2);
        }
        break;
        case USB_REQ_CLEAR_FEATURE:
        {
            ...
            if(USB_EP_HALT == setup->wValue && ep->stalled == OS_TRUE)
            {
                ...
                for (node = ep->request_list.next; node != &ep->request_list; node = node->next)
                {
                    ...
                    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("fired a request\n"));                    
                }
...
{
    static rt_uint8_t * usb_comp_id_desc = OS_NULL;
...
static rt_err_t _dump_setup_packet(ureq_t setup)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("  setup_request : 0x%x\n",
                                setup->request_type));
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("  value         : 0x%x\n", setup->wValue));
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("  length        : 0x%x\n", setup->wLength));
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("  index         : 0x%x\n", setup->wIndex));
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("  request       : 0x%x\n", setup->bRequest));
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("]\n"));
 ...
 *
 * @return an usb device object on success, OS_NULL on fail.
 */
udevice_t rt_usbd_device_new(void)
{
    udevice_t udevice;

    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_device_new\n"));
...
uconfig_t rt_usbd_config_new(void)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_config_new\n"));
 ...
 *
 * @return an usb interface object on success, OS_NULL on fail.
 */
uintf_t rt_usbd_interface_new(udevice_t device, uintf_handler_t handler)
{
    uintf_t intf;

    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_interface_new\n"));
 ...
 *
 * @return an usb alternate setting object on success, OS_NULL on fail.
 */
ualtsetting_t rt_usbd_altsetting_new(rt_size_t desc_size)
{
    ualtsetting_t setting;

    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_altsetting_new\n"));
 ...
 *
 * @return an usb function object on success, OS_NULL on fail.
 */
                              ...
                              ufunction_ops_t ops)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_function_new\n"));
 ...
 * @handler the callback handler of object
 *
 * @return an usb endpoint object on success, OS_NULL on fail.
 */
uep_t rt_usbd_endpoint_new(uep_desc_t ep_desc, udep_handler_t handler)
{
    uep_t ep;

    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_endpoint_new\n"));
 ...
 *
 * @return an usb device object on found or OS_NULL on not found.
 */
 ...
 *
 * @return an usb configuration object on found or OS_NULL on not found.
 */
uconfig_t rt_usbd_find_config(udevice_t device, rt_uint8_t value)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_find_config\n"));
...
uintf_t rt_usbd_find_interface(udevice_t device, rt_uint8_t value, ufunction_t *pfunc)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_find_interface\n"));
 ...
 *
 * @return an usb interface alternate setting object on found or OS_NULL on not found.
 */
ualtsetting_t rt_usbd_find_altsetting(uintf_t intf, rt_uint8_t value)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_find_altsetting\n"));
 ...
 *
 * @return an usb endpoint object on found or OS_NULL on not found.
 */
...
rt_err_t rt_usbd_device_add_config(udevice_t device, uconfig_t cfg)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_device_add_config\n"));
        ...
        for(j=func->intf_list.next; j!=&func->intf_list; j=j->next)
        {
            ...
            for(k=intf->setting_list.next; k!=&intf->setting_list;k=k->next)
            {
                ...
                for(m=altsetting->ep_list.next; m!=&altsetting->ep_list; m=m->next)
                {
                    ...
                    if(rt_usbd_ep_assign(device, ep) != OS_EOK)
                    {
...
{
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_config_add_function\n"));
...
rt_err_t rt_usbd_function_add_interface(ufunction_t func, uintf_t intf)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_function_add_interface\n"));
...
{
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_interface_add_altsetting\n"));
...
{
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_altsetting_add_endpoint\n"));
...
rt_err_t rt_usbd_set_altsetting(uintf_t intf, rt_uint8_t value)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_set_altsetting\n"));
...
rt_err_t rt_usbd_set_config(udevice_t device, rt_uint8_t value)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbd_set_config\n"));
...
rt_size_t rt_usbd_io_request(udevice_t device, uep_t ep, uio_request_t req)
{
    ...
    else
    {
        ...
        ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("suspend a request\n"));
    }            
...
rt_err_t rt_usbd_set_feature(udevice_t device, rt_uint16_t value, rt_uint16_t index)
{
    ...
    {
        ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("set feature remote wakeup\n"));
    }
    else if (value == USB_FEATURE_ENDPOINT_HALT)
    {
        ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("set feature stall\n"));    
...
rt_err_t rt_usbd_clear_feature(udevice_t device, rt_uint16_t value, rt_uint16_t index)
{
    ...
    if (value == USB_FEATURE_DEV_REMOTE_WAKEUP)
    {
        ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("clear feature remote wakeup\n"));
    }
    else if (value == USB_FEATURE_ENDPOINT_HALT)
    {
        ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("clear feature stall\n"));
...
static rt_err_t rt_usbd_ep_assign(udevice_t device, uep_t ep)
{
    ...
    while(device->dcd->ep_pool[i].addr != 0xFF)
    {
            ...
            ep->ep_desc->bmAttributes == device->dcd->ep_pool[i].type && (EP_ADDRESS(ep) & 0x80) == device->dcd->ep_pool[i].dir)
        {
            ...
            ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("assigned %d\n", device->dcd->ep_pool[i].addr));  
            return OS_EOK;
        }
...
static void rt_usbd_thread_entry(void* parameter)
{
    while(1)
    {
        ...
        ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("message type %d\n", msg.type));
        ...
        case USB_MSG_EP0_OUT:
            ...
            break;
        case USB_MSG_RESET:            
            ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("reset %d\n", device->state));
...
ALIGN(OS_ALIGN_SIZE)
static rt_uint8_t usb_thread_stack[DEV_USBD_THREAD_STACK_SZ];
...
rt_err_t rt_usbd_core_init(void)
{
    ...
    rt_thread_init(&usb_thread,
                   "usbd",
                   rt_usbd_thread_entry, OS_NULL,
                   usb_thread_stack, DEV_USBD_THREAD_STACK_SZ,
                   DEV_USBD_THREAD_PRIO, 20);
