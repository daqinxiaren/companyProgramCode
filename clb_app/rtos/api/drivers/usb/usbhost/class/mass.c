...
static rt_err_t _pipe_check(struct uhintf* intf, upipe_t pipe)
{
    ...
    /* check pipe status */
    if(pipe->status == UPIPE_STATUS_OK) return OS_EOK;
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("clean storage in pipe stall\n"));
...
rt_err_t rt_usbh_storage_get_max_lun(struct uhintf* intf, rt_uint8_t* max_lun)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbh_storage_get_max_lun\n"));
    ...
    if(rt_usb_hcd_pipe_xfer(device->hcd, device->pipe_ep0_out, OS_NULL, 0, timeout) != 0)
    {
        return -OS_EIO;
    }
    return OS_EOK;
...
rt_err_t rt_usbh_storage_reset(struct uhintf* intf)
{
    ...
    OS_ASSERT(intf->device != OS_NULL);
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbh_storage_reset\n"));
    ...
    if(rt_usb_hcd_pipe_xfer(device->hcd, device->pipe_ep0_in, OS_NULL, 0, timeout) != 0)
    {
        return -OS_EIO;
    }
    return OS_EOK;
    ...
    rt_uint32_t sector, rt_size_t count, int timeout)
{
    ...
    OS_ASSERT(intf->device != OS_NULL);
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbh_storage_read10\n"));
    ...
    rt_uint32_t sector, rt_size_t count, int timeout)
{
    ...
    OS_ASSERT(intf->device != OS_NULL);
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbh_storage_write10\n"));
...
rt_err_t rt_usbh_storage_request_sense(struct uhintf* intf, rt_uint8_t* buffer)
{
    ...
    OS_ASSERT(intf->device != OS_NULL);
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbh_storage_request_sense\n"));
...
rt_err_t rt_usbh_storage_test_unit_ready(struct uhintf* intf)
{
    ...
    OS_ASSERT(intf->device != OS_NULL);
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbh_storage_test_unit_ready\n"));
    ...
    return rt_usb_bulk_only_xfer(intf, &cmd, OS_NULL, timeout);
}
...
rt_err_t rt_usbh_storage_inquiry(struct uhintf* intf, rt_uint8_t* buffer)
{
    ...
    OS_ASSERT(intf->device != OS_NULL);
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbh_storage_inquiry\n"));
...
rt_err_t rt_usbh_storage_get_capacity(struct uhintf* intf, rt_uint8_t* buffer)
{
    ...
    OS_ASSERT(intf->device != OS_NULL);
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbh_storage_get_capacity\n"));
...
static rt_err_t rt_usbh_storage_enable(void* arg)
{
    ...
    if(stor->pipe_in == OS_NULL || stor->pipe_out == OS_NULL)
    {
...
static rt_err_t rt_usbh_storage_disable(void* arg)
{
    ...
    OS_ASSERT(intf->user_data != OS_NULL);
    OS_ASSERT(intf->device != OS_NULL);

    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbh_storage_stop\n"));
