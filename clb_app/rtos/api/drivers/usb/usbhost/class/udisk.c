...
static int udisk_get_id(void)
{
    ...
    if(i == UDISK_MAX_COUNT) OS_ASSERT(0);
...
//        rt_usbh_clear_feature(intf->device, 0, USB_FEATURE_ENDPOINT_HALT);
    ...
    while((ret = rt_usbh_storage_inquiry(intf, inquiry)) != OS_EOK)
    {
        if(ret == -OS_EIO) return ret;
    ...
    /* wait device ready */
    while((ret = rt_usbh_storage_test_unit_ready(intf)) != OS_EOK)
    {
        if(ret == -OS_EIO) return ret;
    ...
    while((ret = rt_usbh_storage_get_capacity(intf,
        (rt_uint8_t*)stor->capicity)) != OS_EOK)
    {
        if(ret == -OS_EIO) return ret;
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("capicity %d, block size %d\n",
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("read partition table\n"));
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("finished reading partition\n"));
    ...
    for(i=0; i<MAX_PARTITION_COUNT; i++)
    {
        ...
        if (ret == OS_EOK)
        {
            ...
            data->part.lock = rt_sem_create(sname, 1, IPC_FLAG_FIFO);

            /* register sdcard device */
            stor->dev[i].type    = DEVICE_CLASS_BLOCK;
#ifdef ENABLE_DEVICE_OPS
            ...
            rt_device_register(&stor->dev[i], dname, DEVICE_FLAG_RDWR |
                DEVICE_FLAG_REMOVABLE | DEVICE_FLAG_STANDALONE);

            stor->dev_cnt++;
            if (dfs_mount(stor->dev[i].parent.name, UDISK_MOUNTPOINT, "elm",
                0, 0) == 0)
            {
                ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("udisk part %d mount successfully\n", i));
            }
            else
            {
                ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("udisk part %d mount failed\n", i));
            }
        }
        else
        {
            if(i == 0)
            {
                ...
                data->part.size   = 0;
                data->intf = intf;
                data->part.lock = rt_sem_create("sem_ud", 1, IPC_FLAG_FIFO);

                rt_snprintf(dname, 7, "udisk%d", data->udisk_id);

                /* register sdcard device */
                stor->dev[0].type    = DEVICE_CLASS_BLOCK;
#ifdef ENABLE_DEVICE_OPS
                stor->dev[i].ops     = &udisk_device_ops;
#else
                ...
                rt_device_register(&stor->dev[0], dname,
                    DEVICE_FLAG_RDWR | DEVICE_FLAG_REMOVABLE
                    | DEVICE_FLAG_STANDALONE);
