...
static rt_err_t rt_usbh_hub_port_change(uhub_t hub)
{
    ...
    for (i = 0; i < hub->num_ports; i++)
    {
        ...
        if(ret != OS_EOK) continue;

        ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("port %d status 0x%x\n", i + 1, pstatus));
        ...
        if(reconnect)
        {            
            if(hub->child[i] != OS_NULL && hub->child[i]->status != DEV_STATUS_IDLE)
            { 
...
static void rt_usbh_hub_irq(void* context)
{
    ...
    {
        ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB,("hub irq error\n"));
        return;
    }
    
    rt_usbh_hub_port_change(hub);

    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB,("hub int xfer...\n"));

    /* parameter check */
     OS_ASSERT(pipe->inst->hcd != OS_NULL);
...
static rt_err_t rt_usbh_hub_enable(void *arg)
{
    int i = 0;
    rt_err_t ret = OS_EOK;
    uep_desc_t ep_desc = OS_NULL;
    uhub_t hub;
    struct uinstance* device;
    struct uhintf* intf = (struct uhintf*)arg;
    upipe_t pipe_in = OS_NULL;
    int timeout = USB_TIMEOUT_LONG;
    /* paremeter check */
    OS_ASSERT(intf != OS_NULL);
    
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbh_hub_run\n"));
...
static rt_err_t rt_usbh_hub_disable(void* arg)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usbh_hub_stop\n"));
    hub = (uhub_t)intf->user_data;

    for(i=0; i<hub->num_ports; i++)
    {
        if(hub->child[i] != OS_NULL)
            rt_usbh_detach_instance(hub->child[i]);
    }
    
    if(hub != OS_NULL) rt_free(hub);
...
{    
    ...
    {    
        ...
        /* receive message */
        if(rt_mq_recv(usb_mq, &msg, sizeof(struct uhost_msg), WAITING_FOREVER) 
            != OS_EOK ) continue;

        //ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("msg type %d\n", msg.type));
...
void rt_usbh_hub_init(uhcd_t hcd)
{
    ...
    root_hub.is_roothub = OS_TRUE;
    hcd->roothub = &root_hub;
    root_hub.hcd = hcd;
    root_hub.num_ports = hcd->num_ports;
    /* create usb message queue */
    usb_mq = rt_mq_create("usbh", 32, 16, IPC_FLAG_FIFO);
    
    /* create usb hub thread */
    thread = rt_thread_create("usbh", rt_usbh_hub_thread_entry, OS_NULL, 
