 ...
 *
 * @return the allocate instance on successful, or OS_NULL on failure.
 */
...
rt_err_t rt_usbh_attatch_instance(uinst_t device)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("start enumnation\n"));
    ...
    rt_usb_hcd_alloc_pipe(device->hcd, &device->pipe_ep0_out, device, &ep0_out_desc);
    rt_usb_hcd_alloc_pipe(device->hcd, &device->pipe_ep0_in, device, &ep0_in_desc);
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("get device descriptor length %d\n",
                                dev_desc->bLength));
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("Vendor ID 0x%x\n", dev_desc->idVendor));
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("Product ID 0x%x\n", dev_desc->idProduct));
    ...
    {        
        ...
        ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("interface class 0x%x, subclass 0x%x\n", 
        ...
        for(ep_index = 0; ep_index < intf_desc->bNumEndpoints; ep_index++)
        {
            ...
            if(ep_desc != OS_NULL)
            {
                if(rt_usb_hcd_alloc_pipe(device->hcd, &pipe, device, ep_desc) != OS_EOK)
                {
        ...
        if(drv != OS_NULL)
        {
            ...
            device->intf[i]->user_data = OS_NULL;
...
rt_err_t rt_usbh_detach_instance(uinst_t device)
{
    ...
    if (device->cfg_desc) {
        for (i = 0; i < device->cfg_desc->bNumInterfaces; i++)
        {
            if (device->intf[i] == OS_NULL) continue;
            if (device->intf[i]->drv == OS_NULL) continue;

            OS_ASSERT(device->intf[i]->device == device);

            ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("free interface instance %d\n", i));
...
rt_err_t rt_usbh_set_address(uinst_t device)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB, ("rt_usb_set_address\n"));
    ...
    uep_desc_t* ep_desc)
{
    ...
    /* check parameter */
    OS_ASSERT(intf_desc != OS_NULL);
    OS_ASSERT(num < intf_desc->bNumEndpoints);
    *ep_desc = OS_NULL;
...
int rt_usb_hcd_pipe_xfer(uhcd_t hcd, upipe_t pipe, void* buffer, int nbytes, int timeout)
{
    ...
    do
    {
        ENABLE_DEBUG_LOG(ENABLE_DEBUG_USB,("pipe transform remain size,: %d\n", remain_size));
