...
{
#ifdef TOUCH_PIN_IRQ
    if (touch->config.irq_pin.pin == DEV_PIN_NONE)
    {
        return -OS_EINVAL;
    }
...
static void rt_touch_irq_enable(rt_touch_t touch)
{
    ...
    if (touch->config.irq_pin.pin != DEV_PIN_NONE)
    {
        rt_pin_irq_enable(touch->config.irq_pin.pin, OS_TRUE);
    }
#endif
}
...
static void rt_touch_irq_disable(rt_touch_t touch)
{
    ...
    if (touch->config.irq_pin.pin != DEV_PIN_NONE)
    {
        rt_pin_irq_enable(touch->config.irq_pin.pin, OS_FALSE);
    }
#endif
}
...
static rt_err_t rt_touch_control(rt_device_t dev, int cmd, void *args)
{
    ...
    switch (cmd)
    {
    case TOUCH_CTRL_GET_ID:
        if (args)
        {
            result = touch->ops->touch_control(touch, TOUCH_CTRL_GET_ID, args);
        }
        else
        {
            result = -OS_ERROR;
        ...
        break;
    case TOUCH_CTRL_GET_INFO:
        if (args)
        {
            result = touch->ops->touch_control(touch, TOUCH_CTRL_GET_INFO, args);
        }
        else
        {
            result = -OS_ERROR;
        ...
        break;
    case TOUCH_CTRL_SET_MODE:
        result = touch->ops->touch_control(touch, TOUCH_CTRL_SET_MODE, args);

        if (result == OS_EOK)
        {
            rt_uint16_t mode;
            mode  = *(rt_uint16_t*)args;
            if (mode == DEVICE_FLAG_INT_RX)
            {
        ...
        break;
    case TOUCH_CTRL_SET_X_RANGE:
        result = touch->ops->touch_control(touch, TOUCH_CTRL_SET_X_RANGE, args);
        ...
        break;
    case TOUCH_CTRL_SET_Y_RANGE:
        result = touch->ops->touch_control(touch, TOUCH_CTRL_SET_Y_RANGE, args);
        ...
        break;
    case TOUCH_CTRL_DISABLE_INT:
        rt_touch_irq_disable(touch);
        break;
    case TOUCH_CTRL_ENABLE_INT:
...
#endif
    device->type        = DEVICE_CLASS_TOUCH;
    device->rx_indicate = OS_NULL;
    device->tx_complete = OS_NULL;
    device->user_data   = data;

    result = rt_device_register(device, name, flag | DEVICE_FLAG_STANDALONE);
