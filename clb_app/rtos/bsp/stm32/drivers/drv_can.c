...
#if defined (STM32F1XX)/* APB1 36MHz(max) */
...
#elif defined (STM32F4XX)/* APB1 42MHz(max) */
static const struct stm32_baud_rate_tab can_baud_rate_tab[] =
{
    {CAN1MBaud, (CAN_SJW_2TQ | CAN_BS1_7TQ  | CAN_BS2_6TQ | 3)},
    {CAN800kBaud, (CAN_SJW_2TQ | CAN_BS1_8TQ  | CAN_BS2_5TQ | 4)},
    {CAN500kBaud, (CAN_SJW_2TQ | CAN_BS1_7TQ  | CAN_BS2_6TQ | 6)},
...
#elif defined (STM32F7XX)/* APB1 54MHz(max) */
static const struct stm32_baud_rate_tab can_baud_rate_tab[] =
{
...
#elif defined (STM32L4XX)/* APB1 80MHz(max) */
static const struct stm32_baud_rate_tab can_baud_rate_tab[] =
{
...
static rt_err_t _can_control(struct rt_can_device *can, int cmd, void *arg)
{
    ...
    drv_can = (struct stm32_can *)can->parent.user_data;
    OS_ASSERT(drv_can != OS_NULL);

    switch (cmd)
    {
    case DEVICE_CTRL_CLR_INT:
        argval = (rt_uint32_t) arg;
        if (argval == DEVICE_FLAG_INT_RX)
        {
        ...
        else if (argval == DEVICE_FLAG_INT_TX)
        {
            if (CAN1 == drv_can->CanHandle.Instance)
            {
        ...
        else if (argval == DEVICE_CAN_INT_ERR)
        {
            if (CAN1 == drv_can->CanHandle.Instance)
            {
    ...
    case DEV_CAN_CMD_SET_FILTER:
        ...
        break;
    case DEV_CAN_CMD_SET_MODE:
        argval = (rt_uint32_t) arg;
        if (argval != DEV_CAN_MODE_NORMAL &&
                argval != DEV_CAN_MODE_LISEN &&
                argval != DEV_CAN_MODE_LOOPBACK &&
                argval != DEV_CAN_MODE_LOOPBACKANLISEN)
        {
            return -OS_ERROR;
        }
    ...
    case DEV_CAN_CMD_SET_PRIV:
        argval = (rt_uint32_t) arg;
        if (argval != DEV_CAN_MODE_PRIV &&
                argval != DEV_CAN_MODE_NOPRIV)
        {
            return -OS_ERROR;
        }
...
static int _can_sendmsg(struct rt_can_device *can, const void *buf, rt_uint32_t box_num)
{
            ...
            (state == HAL_CAN_STATE_LISTENING))
    {
        ...
        if (DEV_CAN_STDID == pmsg->ide)
        {
            txheader.IDE = CAN_ID_STD;
            OS_ASSERT(IS_CAN_STDID(pmsg->id));
            txheader.StdId = pmsg->id;
        }
        else
        {
            txheader.IDE = CAN_ID_EXT;
            OS_ASSERT(IS_CAN_EXTID(pmsg->id));
            txheader.ExtId = pmsg->id;
        }

        if (DEV_CAN_DTR == pmsg->rtr)
        {
...
static int _can_recvmsg(struct rt_can_device *can, void *buf, rt_uint32_t fifo)
{
    ...
    {
        pmsg->ide = DEV_CAN_STDID;
        pmsg->id = rxheader.StdId;
    }
    else
    {
        pmsg->ide = DEV_CAN_EXTID;
        pmsg->id = rxheader.ExtId;
    }
    /* get type */
    if (CAN_RTR_DATA == rxheader.RTR)
    {
        pmsg->rtr = DEV_CAN_DTR;
    }
    else
    {
        pmsg->rtr = DEV_CAN_RTR;
    }
    /* get len */
...
static void _can_rx_isr(struct rt_can_device *can, rt_uint32_t fifo)
{
    ...
    case CAN_RX_FIFO0:
        ...
        {
            rt_hw_can_isr(can, DEV_CAN_EVENT_RX_IND | fifo << 8);
        }
        ...
        if (__HAL_CAN_GET_FLAG(hcan, CAN_FLAG_FOV0) && __HAL_CAN_GET_IT_SOURCE(hcan, CAN_IT_RX_FIFO0_OVERRUN))
        {
            ...
            rt_hw_can_isr(can, DEV_CAN_EVENT_RXOF_IND | fifo << 8);
        }
        break;
    case CAN_RX_FIFO1:
        /* save to user list */
...
void CAN1_TX_IRQHandler(void)
{
    ...
    if (__HAL_CAN_GET_FLAG(hcan, CAN_FLAG_RQCP0))
    {
        ...
        {
            rt_hw_can_isr(&drv_can1.device, DEV_CAN_EVENT_TX_DONE | 0 << 8);
        }
        else
        {
            rt_hw_can_isr(&drv_can1.device, DEV_CAN_EVENT_TX_FAIL | 0 << 8);
        }
    ...
    else if (__HAL_CAN_GET_FLAG(hcan, CAN_FLAG_RQCP1))
    {
        ...
        {
            rt_hw_can_isr(&drv_can1.device, DEV_CAN_EVENT_TX_DONE | 1 << 8);
        }
        else
        {
            rt_hw_can_isr(&drv_can1.device, DEV_CAN_EVENT_TX_FAIL | 1 << 8);
        }
        /* Write 0 to Clear transmission status flag RQCPx */
    ...
    else if (__HAL_CAN_GET_FLAG(hcan, CAN_FLAG_RQCP2))
    {
        ...
        {
            rt_hw_can_isr(&drv_can1.device, DEV_CAN_EVENT_TX_DONE | 2 << 8);
        }
        else
        {
            rt_hw_can_isr(&drv_can1.device, DEV_CAN_EVENT_TX_FAIL | 2 << 8);
        }
        /* Write 0 to Clear transmission status flag RQCPx */
...
void CAN1_SCE_IRQHandler(void)
{
    ...
    {
    case DEV_CAN_BUS_BIT_PAD_ERR:
        drv_can1.device.status.bitpaderrcnt++;
        break;
    case DEV_CAN_BUS_FORMAT_ERR:
        drv_can1.device.status.formaterrcnt++;
        break;
    case DEV_CAN_BUS_ACK_ERR:/* attention !!! test ack err's unit is transmit unit */
        ...
        if (!READ_BIT(drv_can1.CanHandle.Instance->TSR, CAN_FLAG_TXOK0))
            ...
            rt_hw_can_isr(&drv_can1.device, DEV_CAN_EVENT_TX_FAIL | 2 << 8);
        break;
    case DEV_CAN_BUS_IMPLICIT_BIT_ERR:
    case DEV_CAN_BUS_EXPLICIT_BIT_ERR:
        drv_can1.device.status.biterrcnt++;
        break;
    case DEV_CAN_BUS_CRC_ERR:
...
void CAN2_TX_IRQHandler(void)
{
    ...
    if (__HAL_CAN_GET_FLAG(hcan, CAN_FLAG_RQCP0))
    {
        if (__HAL_CAN_GET_FLAG(hcan, CAN_FLAG_TXOK0))
        {
            rt_hw_can_isr(&drv_can2.device, DEV_CAN_EVENT_TX_DONE | 0 << 8);
        }
        else
        {
            rt_hw_can_isr(&drv_can2.device, DEV_CAN_EVENT_TX_FAIL | 0 << 8);
        }
        /* Write 0 to Clear transmission status flag RQCPx */
        SET_BIT(hcan->Instance->TSR, CAN_TSR_RQCP0);
    }
    else if (__HAL_CAN_GET_FLAG(hcan, CAN_FLAG_RQCP1))
    {
        if (__HAL_CAN_GET_FLAG(hcan, CAN_FLAG_TXOK1))
        {
            rt_hw_can_isr(&drv_can2.device, DEV_CAN_EVENT_TX_DONE | 1 << 8);
        }
        else
        {
            rt_hw_can_isr(&drv_can2.device, DEV_CAN_EVENT_TX_FAIL | 1 << 8);
        }
        /* Write 0 to Clear transmission status flag RQCPx */
        SET_BIT(hcan->Instance->TSR, CAN_TSR_RQCP1);
    }
    else if (__HAL_CAN_GET_FLAG(hcan, CAN_FLAG_RQCP2))
    {
        if (__HAL_CAN_GET_FLAG(hcan, CAN_FLAG_TXOK2))
        {
            rt_hw_can_isr(&drv_can2.device, DEV_CAN_EVENT_TX_DONE | 2 << 8);
        }
        else
        {
            rt_hw_can_isr(&drv_can2.device, DEV_CAN_EVENT_TX_FAIL | 2 << 8);
        }
        /* Write 0 to Clear transmission status flag RQCPx */
        SET_BIT(hcan->Instance->TSR, CAN_TSR_RQCP2);
    }
...
int rt_hw_can_init(void)
{
    ...
    config.privmode = DEV_CAN_MODE_NOPRIV;
    config.ticks = 50;
