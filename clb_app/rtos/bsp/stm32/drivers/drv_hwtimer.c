...
static void pclkx_doubler_get(uint32_t *pclk1_doubler, uint32_t *pclk2_doubler)
{
    ...
    OS_ASSERT(pclk1_doubler != OS_NULL);
    OS_ASSERT(pclk1_doubler != OS_NULL);
...
static void timer_init(struct rt_hwtimer_device *timer, rt_uint32_t state)
{
    ...
    struct stm32_hwtimer *tim_device = OS_NULL;
...
#if defined(STM32F1XX) || defined(STM32L4XX) || defined(STM32F0XX) || defined(STM32G0XX) || defined(SOC_SERIES_STM32MP1) || defined(SOC_SERIES_STM32WB)
...
#elif defined(STM32F0XX) || defined(SOC_SERIES_STM32MP1) 
...
static int stm32_hwtimer_init(void)
{
    ...
    for (i = 0; i < sizeof(stm32_hwtimer_obj) / sizeof(stm32_hwtimer_obj[0]); i++)
    {
        ...
        if (rt_device_hwtimer_register(&stm32_hwtimer_obj[i].time_device, stm32_hwtimer_obj[i].name, &stm32_hwtimer_obj[i].tim_handle) == OS_EOK)
        {
