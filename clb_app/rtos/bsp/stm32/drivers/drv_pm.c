...
static void uart_console_reconfig(void)
{
    ...
    rt_device_control(rt_console_get_device(), DEVICE_CTRL_CONFIG, &config);
}
...
static void sleep(struct rt_pm *pm, uint8_t mode)
{
    switch (mode)
    {
    case POWER_SLEEP_MODE_NONE:
        break;

    case POWER_SLEEP_MODE_IDLE:
        // __WFI();
        break;

    case POWER_SLEEP_MODE_LIGHT:
        if (pm->run_mode == POWER_RUN_MODE_LOW_SPEED)
        {
    ...
    case POWER_SLEEP_MODE_STANDBY:
        /* Enter STANDBY mode */
        HAL_PWR_EnterSTANDBYMode();
        break;

    case POWER_SLEEP_MODE_SHUTDOWN:
...
static uint8_t run_speed[POWER_RUN_MODE_MAX][2] =
{
    {80, 0},
    {80, 1},
    {24, 2},
...
static void run(struct rt_pm *pm, uint8_t mode)
{
    ...
    static char *run_str[] = POWER_RUN_MODE_NAMES;
    ...
    switch (mode)
    {
    case POWER_RUN_MODE_HIGH_SPEED:
    case POWER_RUN_MODE_NORMAL_SPEED:
        HAL_PWREx_DisableLowPowerRunMode();
        SystemClock_80M();
        /* Configure the main internal regulator output voltage (Range1 by default)*/
        HAL_PWREx_ControlVoltageScaling(PWR_REGULATOR_VOLTAGE_SCALE1);
        break;
    case POWER_RUN_MODE_MEDIUM_SPEED:
        HAL_PWREx_DisableLowPowerRunMode();
        SystemClock_24M();
        /* Configure the main internal regulator output voltage */
        HAL_PWREx_ControlVoltageScaling(PWR_REGULATOR_VOLTAGE_SCALE2);
        break;
    case POWER_RUN_MODE_LOW_SPEED:
        SystemClock_2M();
        /* Enter LP RUN mode */
...
static rt_tick_t stm32l4_pm_tick_from_os_tick(rt_tick_t tick)
{
    ...
    return (freq * tick / TICK_PER_SECOND);
}
...
static rt_tick_t stm32l4_os_tick_from_pm_tick(rt_uint32_t tick)
{
    ...
    ret = (tick * TICK_PER_SECOND + os_tick_remain) / freq;

    os_tick_remain += (tick * TICK_PER_SECOND);
    os_tick_remain %= freq;
...
int drv_pm_hw_init(void)
{
    ...
    timer_mask = 1UL << POWER_SLEEP_MODE_DEEP;

    /* initialize system pm module */
    rt_system_pm_init(&_ops, timer_mask, OS_NULL);
