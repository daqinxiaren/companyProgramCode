...
static rt_err_t drv_lcd_control(struct rt_device *device, int cmd, void *args)
{
    ...
    case GRAPHIC_CTRL_RECT_UPDATE:
    {
        ...
        rt_sem_take(&_lcd.lcd_lock, TICK_PER_SECOND / 20);
...
rt_err_t stm32_lcd_init(struct drv_lcd_device *lcd)
{
    ...
    if (lcd->lcd_info.pixel_format == GRAPHIC_PIXEL_FORMAT_RGB565)
    {
        pLayerCfg.PixelFormat = LTDC_PIXEL_FORMAT_RGB565;
    }
    else if (lcd->lcd_info.pixel_format == GRAPHIC_PIXEL_FORMAT_ARGB888)
    {
        pLayerCfg.PixelFormat = LTDC_PIXEL_FORMAT_ARGB8888;
    }
    else if (lcd->lcd_info.pixel_format == GRAPHIC_PIXEL_FORMAT_RGB888)
    {
...
#if defined(LCD_BACKLIGHT_ENABLE_PWM)
...
#elif defined(LCD_BACKLIGHT_ENABLE_GPIO)
void turn_on_lcd_backlight(void)
{
...
int drv_lcd_hw_init(void)
{
    ...
    result = rt_sem_init(&_lcd.lcd_lock, "lcd_lock", 0, IPC_FLAG_FIFO);
    if (result != OS_EOK)
    {
    ...
    if (_lcd.lcd_info.framebuffer == OS_NULL || _lcd.back_buf == OS_NULL || _lcd.front_buf == OS_NULL)
    {
    ...
    device->type    = DEVICE_CLASS_GRAPHIC;
#ifdef ENABLE_DEVICE_OPS
    device->ops     = &lcd_ops;
#else
    ...
    /* register lcd device */
    rt_device_register(device, "lcd", DEVICE_FLAG_RDWR);

    /* init stm32 LTDC */
    if (stm32_lcd_init(&_lcd) != OS_EOK)
    {
        result = -OS_ERROR;
        goto __exit;
    }
...
int lcd_test()
{
    ...
    while (1)
    {
        ...
        lcd->parent.control(&lcd->parent, GRAPHIC_CTRL_RECT_UPDATE, OS_NULL);
