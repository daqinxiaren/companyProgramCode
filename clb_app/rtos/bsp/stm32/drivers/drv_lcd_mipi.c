...
rt_err_t ltdc_init(void)
{
    ...
    stm32_mipi_lcd_config(GRAPHIC_PIXEL_FORMAT_ARGB888);
...
static rt_err_t stm32_lcd_init(rt_device_t device)
{
    ...
    lcd.info.pixel_format   = GRAPHIC_PIXEL_FORMAT_ARGB888;
...
int rt_hw_lcd_init(void)
{
    ...
    lcd.parent.type    = DEVICE_CLASS_GRAPHIC;
    lcd.parent.init    = stm32_lcd_init;
    lcd.parent.open    = OS_NULL;
    lcd.parent.close   = OS_NULL;
    lcd.parent.read    = OS_NULL;
    lcd.parent.write   = OS_NULL;
    lcd.parent.control = stm32_lcd_control;

    lcd.parent.user_data = (void *)&lcd.info;

    ret = rt_device_register(&lcd.parent, "lcd", DEVICE_FLAG_RDWR);
...
OS_WEAK void stm32_mipi_lcd_init(void)
{
    rt_kprintf("please Implementation function %s\n", __func__);
}

OS_WEAK void stm32_mipi_lcd_config(rt_uint32_t pixel_format)
{
    rt_kprintf("please Implementation function %s\n", __func__);
}

OS_WEAK void stm32_mipi_display_on(void)
{
    rt_kprintf("please Implementation function %s\n", __func__);
}

OS_WEAK void stm32_mipi_display_off(void)
{
    rt_kprintf("please Implementation function %s\n", __func__);
}

#endif /* ENABLE_LCD_MIPI */
