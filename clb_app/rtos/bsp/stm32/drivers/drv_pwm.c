...
static rt_err_t drv_pwm_get(TIM_HandleTypeDef *htim, struct rt_pwm_configuration *configuration)
{
...
#elif defined(STM32F1XX) || defined(STM32F0XX) || defined(STM32G0XX)
    if (0)
#endif
    {
    ...
    {
#if defined(STM32L4XX) || defined(STM32F0XX) || defined(STM32G0XX)
...
static rt_err_t drv_pwm_control(struct rt_device_pwm *device, int cmd, void *arg)
{
    ...
    {
    case PWM_CMD_ENABLE:
        return drv_pwm_enable(htim, configuration, OS_TRUE);
    case PWM_CMD_DISABLE:
        return drv_pwm_enable(htim, configuration, OS_FALSE);
    case PWM_CMD_SET:
...
static int stm32_pwm_init(void)
{
    ...
    {
        /* pwm init */
        if (stm32_hw_pwm_init(&stm32_pwm_obj[i]) != OS_EOK)
        {
        ...
        else
        {
            ...
            /* register pwm device */
            if (rt_device_pwm_register(&stm32_pwm_obj[i].pwm_device, stm32_pwm_obj[i].name, &drv_ops, &stm32_pwm_obj[i].tim_handle) == OS_EOK)
            {
