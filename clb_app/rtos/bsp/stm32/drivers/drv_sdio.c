...
#define HW_SDIO_LOCK(_sdio)   rt_mutex_take(&_sdio->mutex, WAITING_FOREVER)
#define HW_SDIO_UNLOCK(_sdio) rt_mutex_release(&_sdio->mutex);
...
static void rthw_sdio_wait_completed(struct rthw_sdio *sdio)
{
    ...
    if (rt_event_recv(&sdio->event, 0xffffffff, EVENT_FLAG_OR | EVENT_FLAG_CLEAR,
                      rt_tick_from_millisecond(5000), &status) != OS_EOK)
    {
...
static void rthw_sdio_request(struct rt_mmcsd_host *host, struct rt_mmcsd_req *req)
{
    ...
    if (req->cmd != OS_NULL)
    {
        ...
        if ((data != OS_NULL) && (data->flags & DATA_DIR_READ) && ((rt_uint32_t)data->buf & (SDIO_ALIGN_LEN - 1)))
        {
...
{
    struct rt_mmcsd_host *host;
    struct rthw_sdio *sdio = OS_NULL;

    if ((sdio_des == OS_NULL) || (sdio_des->txconfig == OS_NULL) || (sdio_des->rxconfig == OS_NULL))
    {
        LOG_E("L:%d F:%s %s %s %s",
              (sdio_des == OS_NULL ? "sdio_des is NULL" : ""),
    ...
    sdio->sdio_des.hw_sdio = (sdio_des->hw_sdio == OS_NULL ? (struct stm32_sdio *)SDIO_BASE_ADDRESS : sdio_des->hw_sdio);
    sdio->sdio_des.clk_get = (sdio_des->clk_get == OS_NULL ? stm32_sdio_clk_get : sdio_des->clk_get);

    rt_event_init(&sdio->event, "sdio", IPC_FLAG_FIFO);
    rt_mutex_init(&sdio->mutex, "sdio", IPC_FLAG_FIFO);
...
#elif defined(STM32F4XX) || defined(STM32F7XX) || defined(STM32L4XX) || defined(STM32F2XX)
