 ...
 * @note
**/
uint8_t bsp_internal_flash_read(uint8_t *data, uint32_t size)
{
	int result_size = 0;

   if(data == OS_NULL || size == 0 || size > BSP_FLASH_SECTOR_BYTE_SIZE) {
	   return 1;
    }
 ...
 * @note
**/
uint8_t bsp_internal_flash_write(uint8_t *data, uint32_t size)
{
	int result_size = 0;
