...
int strtest_main(void)
{
    ...
    rt_kprintf("Reading test_strs.ini\n");
    rt_kprintf("=====================\n");
    fp = fopen("/example.ini", "r");
    ...
    if (result < 0)
    {
        rt_kprintf("Error getting entry from test_strs.ini\n");
    }
    rt_kprintf("Reading test.ini end\n");
    return 0;
}
//INIT_ENV_EXPORT(strtest_main);
/**@}*/
