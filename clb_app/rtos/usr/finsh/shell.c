...
static char *finsh_prompt_custom = OS_NULL;
...
#define _PROMPT     "finsh "
    static char finsh_prompt[CONSOLE_BUFFER_SIZE + 1] = {0};
...
#if defined(ENABLE_DFS) && defined(DFS_ENABLE_WORKDIR)
    /* get current working directory */
    getcwd(&finsh_prompt[rt_strlen(finsh_prompt)], CONSOLE_BUFFER_SIZE - rt_strlen(finsh_prompt));
#endif
...
static int finsh_getchar(void)
{
    ...
    while (rt_device_read(shell->device, -1, &ch, 1) != 1)
        rt_sem_take(&shell->rx_sem, WAITING_FOREVER);
...
#if !defined(ENABLE_POSIX) && defined(ENABLE_DEVICE)
...
void finsh_set_device(const char *device_name)
{
    ...
    if (rt_device_open(dev, DEVICE_OFLAG_RDWR | DEVICE_FLAG_INT_RX | \
                       DEVICE_FLAG_STREAM) == OS_EOK)
    {
        if (shell->device != OS_NULL)
        {
            /* close old finsh device */
            rt_device_close(shell->device);
            rt_device_set_rx_indicate(shell->device, OS_NULL);
        }
...
#ifdef FINSH_ENABLE_AUTH
/**
 * set a new password for finsh
 *
 * @param password new password
 *
 * @return result, OS_EOK on OK, -OS_ERROR on the new password length is less than
...
{
    int ch;
    rt_bool_t input_finish = OS_FALSE;
    ...
    while (1)
    {
        ...
        else
        {
            ...
            rt_thread_delay(2 * TICK_PER_SECOND);
            rt_kprintf("Sorry, try again.\n");
            cur_pos = 0;
            input_finish = OS_FALSE;
            rt_memset(password, '\0', FINSH_PASSWORD_MAX);
        }
    }
}
#endif /* FINSH_ENABLE_AUTH */
...
void finsh_thread_entry(void *parameter)
{
    ...
    {
        if (finsh_set_password(FINSH_DEFAULT_PASSWORD) != OS_EOK)
        {
