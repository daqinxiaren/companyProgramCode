...
long list_thread(void)
{
    ...
    rt_list_t *next = (rt_list_t*)OS_NULL;
            ...
            for (i = 0; i < find_arg.nr_out; i++)
            {
                ...
                thread = (struct rt_thread*)obj;
                {
                    ...
                    if (thread->oncpu != OS_CPU_DETACHED)
                        rt_kprintf("%-*.*s %3d %3d ", maxlen, OS_NAME_MAX, thread->name, thread->oncpu, thread->current_priority);
                    else
                        rt_kprintf("%-*.*s N/A %3d ", maxlen, OS_NAME_MAX, thread->name, thread->current_priority);

#else
                    rt_kprintf("%-*.*s %3d ", maxlen, OS_NAME_MAX, thread->name, thread->current_priority);
#endif /*ENABLE_SMP*/
                    stat = (thread->stat & THREAD_STAT_MASK);
                    if (stat == THREAD_READY)        rt_kprintf(" ready  ");
                    else if (stat == THREAD_SUSPEND) rt_kprintf(" suspend");
                    else if (stat == THREAD_INIT)    rt_kprintf(" init   ");
                    else if (stat == THREAD_CLOSE)   rt_kprintf(" close  ");
                    else if (stat == THREAD_RUNNING) rt_kprintf(" running");
            ...
            for (i = 0; i < find_arg.nr_out; i++)
            {
                ...
                else
                {
                    rt_kprintf("%-*.*s  0x%08x 0\n",
                            maxlen, OS_NAME_MAX, e->parent.parent.name, e->set);
                }
            }
        }
    }
...
long list_msgqueue(void)
{
    ...
    list_find_init(&find_arg, OBJECT_CLASS_MESSAGE_QUEUE, obj_list, sizeof(obj_list)/sizeof(obj_list[0]));
...
long list_memheap(void)
{
    ...
    list_find_init(&find_arg, OBJECT_CLASS_MEMORY_HEAP, obj_list, sizeof(obj_list)/sizeof(obj_list[0]));
...
long list_mempool(void)
{
    ...
    list_find_init(&find_arg, OBJECT_CLASS_MEMORY_POOL, obj_list, sizeof(obj_list)/sizeof(obj_list[0]));
...
long list(void)
{
...
#if defined(FINSH_ENABLE_DESCRIPTION) && defined(FINSH_ENABLE_SYMTAB)
    ...
    rt_kprintf("--Variable List:\n");
    {
        ...
        {
#ifdef FINSH_ENABLE_DESCRIPTION
            rt_kprintf("%-16s -- %s\n", index->name, index->desc);
#else
            rt_kprintf("%s\n", index->name);
#endif
...
#if defined(FINSH_ENABLE_SYMTAB) && !defined(FINSH_ENABLE_MSH_ONLY)
static int dummy = 0;
