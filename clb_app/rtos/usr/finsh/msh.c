...
#ifdef FINSH_ENABLE_MSH
#ifdef FINSH_ENABLE_MSH_ONLY
rt_bool_t msh_is_used(void)
{
    return OS_TRUE;
}
#else
#ifdef FINSH_ENABLE_MSH_DEFAULT
static rt_bool_t __msh_state = OS_TRUE;
#else
static rt_bool_t __msh_state = OS_FALSE;
#endif
rt_bool_t msh_is_used(void)
{
    return __msh_state;
...
{
    struct finsh_syscall *index;
    cmd_function_t cmd_func = OS_NULL;
...
#if defined(ENABLE_MODULE) && defined(ENABLE_DFS)
...
#if defined(ENABLE_LWP) && defined(ENABLE_DFS)
...
void msh_auto_complete_path(char *path)
{
    ...
    if (full_path == OS_NULL) return; /* out of memory */
    ...
    if (index != OS_NULL)
    {
        ...
        *ptr = '\0';
        ...
        if (dir == OS_NULL) /* open directory failed! */
        {
            rt_free(full_path);
            return;
        }
