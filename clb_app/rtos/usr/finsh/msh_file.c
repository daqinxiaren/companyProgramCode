...
#if defined(FINSH_ENABLE_MSH) && defined(ENABLE_DFS)
...
int msh_exec_script(const char *cmd_line, int size)
{
    ...
    if (pg_name == OS_NULL) return -OS_ENOMEM;

    /* copy command0 */
    memcpy(pg_name, cmd_line, cmd_length);
    pg_name[cmd_length] = '\0';

    if (strstr(pg_name, ".sh") != OS_NULL || strstr(pg_name, ".SH") != OS_NULL)
    {
        /* try to open program */
    ...
    if (fd >= 0)
    {
        ...
        line_buf = (char *) rt_malloc(CONSOLE_BUFFER_SIZE);
        if (line_buf == OS_NULL) 
        {
            close(fd);
            return -OS_ENOMEM;
        ...
        do
        {
            length = msh_readline(fd, line_buf, CONSOLE_BUFFER_SIZE);
            if (length > 0)
            {
                char ch = '\0';
                int index;
...
int cmd_mv(int argc, char **argv)
{
    ...
    else
    {
        ...
        rename(argv[1], dest);
        if (dest != OS_NULL && dest != argv[2]) rt_free(dest);
    }
...
#endif /* defined(FINSH_ENABLE_MSH) && defined(ENABLE_DFS) */
