...
/* exception hook */
static rt_err_t (*rt_exception_hook)(void *context) = OS_NULL;
                             ...
                             void       *texit)
{
    ...
    stk  = (rt_uint8_t *)OS_ALIGN_DOWN((rt_uint32_t)stk, 8);
...
void rt_hw_hard_fault_exception(struct exception_info *exception_info)
{
    ...
    if (rt_exception_hook != OS_NULL)
    {
        rt_err_t result;

        result = rt_exception_hook(exception_stack);
        if (result == OS_EOK) return;
    }
 ...
 * shutdown CPU
 */
OS_WEAK void rt_hw_cpu_shutdown(void)
{
    rt_kprintf("shutdown...\n");
 ...
 * reset CPU
 */
OS_WEAK void rt_hw_cpu_reset(void)
{
    SCB_AIRCR = SCB_RESET_VALUE;
}

#ifdef ENABLE_CPU_FFS
/**
