...
void rt_hw_cpu_icache_ops(int ops, void* addr, int size)
{
    ...
    if (ops & HARDWARE_CACHE_INVALIDATE)
    {
        __DSB();
        while (size_byte > 0)
        {
...
void rt_hw_cpu_dcache_ops(int ops, void* addr, int size)
{
    ...
    if (ops & (HARDWARE_CACHE_FLUSH | HARDWARE_CACHE_INVALIDATE))
    {
        SCB_CleanInvalidateDCache_by_Addr((uint32_t *)startAddr, size_byte);
    }
    else if (ops & HARDWARE_CACHE_FLUSH)
    {
        SCB_CleanDCache_by_Addr((uint32_t *)startAddr, size_byte);
    }
    else if (ops & HARDWARE_CACHE_INVALIDATE)
    {
