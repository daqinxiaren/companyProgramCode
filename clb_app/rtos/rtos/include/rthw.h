...
enum HARDWARE_CACHE_OPS
{
    HARDWARE_CACHE_FLUSH      = 0x01,
    HARDWARE_CACHE_INVALIDATE = 0x02,
};
...
#define __HARDWARE_SPIN_LOCK_INITIALIZER(lockname) {0}

#define __HARDWARE_SPIN_LOCK_UNLOCKED(lockname)    \
    (rt_hw_spinlock_t) __HARDWARE_SPIN_LOCK_INITIALIZER(lockname)

#define OS_DEFINE_SPINLOCK(x)  rt_hw_spinlock_t x = __HARDWARE_SPIN_LOCK_UNLOCKED(x)
#define OS_DECLARE_SPINLOCK(x)
...
#define OS_DECLARE_SPINLOCK(x)    rt_ubase_t x
