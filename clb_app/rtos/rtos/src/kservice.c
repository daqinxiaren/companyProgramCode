 ...
 * 2013-06-24     Bernard      remove rt_kprintf if ENABLE_CONSOLE is not defined.
...
#if defined(ENABLE_DEVICE) && defined(ENABLE_CONSOLE)
static rt_device_t _console_device = OS_NULL;
#endif
 ...
 *
 * @return the first occurrence of a s2 in s1, or OS_NULL if no found.
 */
...
void rt_show_version(void)
{
    ...
    rt_kprintf(" / | \\     %d.%d.%d build %s\n",
               OS_VERSION, OS_SUBVERSION, OS_REVISION, __DATE__);
    rt_kprintf(" 2006 - 2020 Copyright by rt-thread team\n");
}
OS_MODE_EXPORT(rt_show_version);
 ...
 *
 * @return the device using in console or OS_NULL
 */
rt_device_t rt_console_get_device(void)
{
    return _console_device;
}
OS_MODE_EXPORT(rt_console_get_device);
 ...
 *
 * @return the old console device handler on successful, or OS_NULL on failure.
 */
rt_device_t rt_console_set_device(const char *name)
{
    ...
    if (new_device == old_device) return OS_NULL;
    
    if (new_device != OS_NULL)
    {
        if (_console_device != OS_NULL)
        {
        ...
        rt_device_open(new_device, DEVICE_OFLAG_RDWR | DEVICE_FLAG_STREAM);
        _console_device = new_device;
    }

    return old_device;
}
OS_MODE_EXPORT(rt_console_set_device);
#endif

OS_WEAK void rt_hw_console_output(const char *str)
{
    /* empty console output */
}
OS_MODE_EXPORT(rt_hw_console_output);
...
void rt_kputs(const char *str)
{
    ...
    else
    {
        ...
        _console_device->open_flag |= DEVICE_FLAG_STREAM;
...
void rt_kprintf(const char *fmt, ...)
{
    ...
    if (length > CONSOLE_BUFFER_SIZE - 1)
        length = CONSOLE_BUFFER_SIZE - 1;
#ifdef ENABLE_DEVICE
    if (_console_device == OS_NULL)
    {
 ...
 *
 * @return the allocated memory block on successful, otherwise returns OS_NULL
 */
...
OS_MODE_EXPORT(rt_malloc_align);
...
OS_MODE_EXPORT(rt_free_align);
#endif

#ifndef ENABLE_CPU_FFS
...
/**
 * This function will set a hook function to OS_ASSERT(EX). It will run when the expression is false.
 *
...
OS_MODE_EXPORT(rt_assert_handler);
#endif /* ENABLE_DEBUG */

#if !defined (ENABLE_NEWLIB) && defined (ENABLE_MINILIBC) && defined (__GNUC__)
#include <sys/types.h>
