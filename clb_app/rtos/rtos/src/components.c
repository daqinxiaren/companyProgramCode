 ...
 *                             in some IDEs.
 * 2015-07-29     Arda.Fu      Add support to use ENABLE_USER_MAIN with IAR
...
#ifdef ENABLE_USER_MAIN
#ifndef MAIN_THREAD_STACK_SIZE
#define MAIN_THREAD_STACK_SIZE     2048
#endif
#ifndef MAIN_THREAD_PRIORITY
#define MAIN_THREAD_PRIORITY       (THREAD_PRIORITY_MAX / 3)
#endif
#endif
...
ALIGN(8)
static rt_uint8_t main_stack[MAIN_THREAD_STACK_SIZE];
...
void rt_application_init(void)
{
    ...
    tid = rt_thread_create("main", main_thread_entry, OS_NULL,
    ...
    tid = &main_thread;
    result = rt_thread_init(tid, "main", main_thread_entry, OS_NULL,
                            main_stack, sizeof(main_stack), MAIN_THREAD_PRIORITY, 20);
    OS_ASSERT(result == OS_EOK);

    /* if not define ENABLE_MEMORY_HEAP, using to eliminate the warning */
    (void)result;
#endif
