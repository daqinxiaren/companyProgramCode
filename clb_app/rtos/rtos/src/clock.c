...
rt_tick_t rt_tick_from_millisecond(rt_int32_t ms)
{
    ...
    if (ms < 0)
    {
        tick = (rt_tick_t)WAITING_FOREVER;
    }
    else
    {
...
OS_MODE_EXPORT(rt_tick_from_millisecond);
 ...
 */
OS_WEAK rt_tick_t rt_tick_get_millisecond(void)
{
#if 1000 % TICK_PER_SECOND == 0u
    return rt_tick_get() * (1000u / TICK_PER_SECOND);
#else
