 ...
 * 2012-12-25     Bernard      return OS_EOK if the device interface not exist.
                            ...
                            rt_uint16_t flags)
{
    ...
    if (rt_device_find(name) != OS_NULL)
        return -OS_ERROR;

    rt_object_init(&(dev->parent), OBJECT_CLASS_DEVICE, name);
    dev->flag = flags;
    dev->ref_count = 0;
    dev->open_flag = 0;
...
OS_MODE_EXPORT(rt_device_register);
...
rt_err_t rt_device_unregister(rt_device_t dev)
{
    ...
    OS_ASSERT(rt_object_get_type(&dev->parent) == OBJECT_CLASS_DEVICE);
    OS_ASSERT(rt_object_is_systemobject(&dev->parent));

    rt_object_detach(&(dev->parent));

    return OS_EOK;
}
OS_MODE_EXPORT(rt_device_unregister);
...
{
    return (rt_device_t)rt_object_find(name, OBJECT_CLASS_DEVICE);
}
OS_MODE_EXPORT(rt_device_find);
 ...
 *
 * @return the allocated device object, or OS_NULL when failed.
 */
rt_device_t rt_device_create(int type, int attach_size)
{
    ...
    size = OS_ALIGN(sizeof(struct rt_device), OS_ALIGN_SIZE);
    attach_size = OS_ALIGN(attach_size, OS_ALIGN_SIZE);
    /* use the total size */
    size += attach_size;
...
OS_MODE_EXPORT(rt_device_create);
...
void rt_device_destroy(rt_device_t dev)
{
    OS_ASSERT(dev != OS_NULL);
    OS_ASSERT(rt_object_get_type(&dev->parent) == OBJECT_CLASS_DEVICE);
    OS_ASSERT(rt_object_is_systemobject(&dev->parent) == OS_FALSE);
...
OS_MODE_EXPORT(rt_device_destroy);
#endif
...
OS_MODE_EXPORT(rt_device_close);
                         ...
                         rt_size_t   size)
{
    ...
    if (dev->ref_count == 0)
    {
        rt_set_errno(-OS_ERROR);
        return 0;
    }

    /* call device_read interface */
    if (device_read != OS_NULL)
    {
        return device_read(dev, pos, buffer, size);
    }

    /* set error code */
    rt_set_errno(-OS_ENOSYS);

    return 0;
}
OS_MODE_EXPORT(rt_device_read);
...
OS_MODE_EXPORT(rt_device_write);
...
OS_MODE_EXPORT(rt_device_control);
...
OS_MODE_EXPORT(rt_device_set_rx_indicate);
...
OS_MODE_EXPORT(rt_device_set_tx_complete);
