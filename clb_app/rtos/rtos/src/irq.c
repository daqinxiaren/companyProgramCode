...
void rt_interrupt_enter(void)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_IRQ, ("irq coming..., irq nest:%d\n",
                                rt_interrupt_nest));

    level = rt_hw_interrupt_disable();
    rt_interrupt_nest ++;
    OBJECT_HOOK_CALL(rt_interrupt_enter_hook,());
    rt_hw_interrupt_enable(level);
}
OS_MODE_EXPORT(rt_interrupt_enter);
...
void rt_interrupt_leave(void)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_IRQ, ("irq leave, irq nest:%d\n",
                                rt_interrupt_nest));

    level = rt_hw_interrupt_disable();
    rt_interrupt_nest --;
    OBJECT_HOOK_CALL(rt_interrupt_leave_hook,());
    rt_hw_interrupt_enable(level);
}
OS_MODE_EXPORT(rt_interrupt_leave);
 ...
 */
OS_WEAK rt_uint8_t rt_interrupt_get_nest(void)
{
    rt_uint8_t ret;
    rt_base_t level;
...
OS_MODE_EXPORT(rt_interrupt_get_nest);

OS_MODE_EXPORT(rt_hw_interrupt_disable);
OS_MODE_EXPORT(rt_hw_interrupt_enable);
