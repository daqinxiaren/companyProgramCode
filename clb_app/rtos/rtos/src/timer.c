...
/* hard timer list */
static rt_list_t rt_timer_list[TIMER_SKIP_LIST_LEVEL];

#ifdef ENABLE_TIMER_SOFT
...
/* soft timer list */
static rt_list_t rt_soft_timer_list[TIMER_SKIP_LIST_LEVEL];
static struct rt_thread timer_thread;
ALIGN(OS_ALIGN_SIZE)
static rt_uint8_t timer_thread_stack[TIMER_THREAD_STACK_SIZE];
#endif
                           ...
                           rt_uint8_t flag)
{
    ...
    /* set deactivated */
    timer->parent.flag &= ~TIMER_FLAG_ACTIVATED;
...
static rt_tick_t rt_timer_list_next_timeout(rt_list_t timer_list[])
{
    ...
    rt_tick_t timeout_tick = TICK_MAX;
...
void rt_timer_dump(rt_list_t timer_heads[])
{
    ...
    for (list = timer_heads[TIMER_SKIP_LIST_LEVEL - 1].next;
         list != &timer_heads[TIMER_SKIP_LIST_LEVEL - 1];
         list = list->next)
    {
        struct rt_timer *timer = rt_list_entry(list,
                                               struct rt_timer,
                                               row[TIMER_SKIP_LIST_LEVEL - 1]);
                   ...
                   rt_uint8_t  flag)
{
    ...
    rt_object_init((rt_object_t)timer, OBJECT_CLASS_TIMER, name);

    _rt_timer_init(timer, timeout, parameter, time, flag);
}
OS_MODE_EXPORT(rt_timer_init);
...
rt_err_t rt_timer_detach(rt_timer_t timer)
{
    ...
    /* timer check */
    OS_ASSERT(timer != OS_NULL);
    OS_ASSERT(rt_object_get_type(&timer->parent) == OBJECT_CLASS_TIMER);
    OS_ASSERT(rt_object_is_systemobject(&timer->parent));
...
OS_MODE_EXPORT(rt_timer_detach);
                           ...
                           rt_uint8_t  flag)
{
    ...
    /* allocate a object */
    timer = (struct rt_timer *)rt_object_allocate(OBJECT_CLASS_TIMER, name);
    if (timer == OS_NULL)
    {
        return OS_NULL;
    }
...
OS_MODE_EXPORT(rt_timer_create);
...
rt_err_t rt_timer_delete(rt_timer_t timer)
{
    ...
    OS_ASSERT(timer != OS_NULL);
    OS_ASSERT(rt_object_get_type(&timer->parent) == OBJECT_CLASS_TIMER);
    OS_ASSERT(rt_object_is_systemobject(&timer->parent) == OS_FALSE);
...
OS_MODE_EXPORT(rt_timer_delete);
#endif
...
rt_err_t rt_timer_start(rt_timer_t timer)
{
    ...
    register rt_base_t level;
    rt_list_t *row_head[TIMER_SKIP_LIST_LEVEL];
    unsigned int tst_nr;
    ...
    OBJECT_HOOK_CALL(rt_object_take_hook, (&(timer->parent)));

    /*
     * get timeout tick,
     * the max timeout tick shall not great than TICK_MAX/2
     */
    OS_ASSERT(timer->init_tick < TICK_MAX / 2);
    timer->timeout_tick = rt_tick_get() + timer->init_tick;

#ifdef ENABLE_TIMER_SOFT
    if (timer->parent.flag & TIMER_FLAG_SOFT_TIMER)
    {
    ...
    for (row_lvl = 0; row_lvl < TIMER_SKIP_LIST_LEVEL; row_lvl++)
    {
             ...
             row_head[row_lvl]  = row_head[row_lvl]->next)
        {
            ...
            else if ((t->timeout_tick - timer->timeout_tick) < TICK_MAX / 2)
            {
                break;
            }
        }
    ...
    rt_list_insert_after(row_head[TIMER_SKIP_LIST_LEVEL - 1],
                         &(timer->row[TIMER_SKIP_LIST_LEVEL - 1]));
    for (row_lvl = 2; row_lvl <= TIMER_SKIP_LIST_LEVEL; row_lvl++)
    {
        if (!(tst_nr & TIMER_SKIP_LIST_MASK))
            rt_list_insert_after(row_head[TIMER_SKIP_LIST_LEVEL - row_lvl],
                                 &(timer->row[TIMER_SKIP_LIST_LEVEL - row_lvl]));
        else
            break;
    ...
    if (timer->parent.flag & TIMER_FLAG_SOFT_TIMER)
    {
        ...
        if ((soft_timer_status == RT_SOFT_TIMER_IDLE) &&
           ((timer_thread.stat & THREAD_STAT_MASK) == THREAD_SUSPEND))
        {
...
rt_err_t rt_timer_stop(rt_timer_t timer)
{
    ...
    OBJECT_HOOK_CALL(rt_object_put_hook, (&(timer->parent)));
...
rt_err_t rt_timer_control(rt_timer_t timer, int cmd, void *arg)
{
    ...
    case TIMER_CTRL_SET_ONESHOT:
        timer->parent.flag &= ~TIMER_FLAG_PERIODIC;
        break;
    ...
    case TIMER_CTRL_GET_STATE:
        if(timer->parent.flag & TIMER_FLAG_ACTIVATED)
        {
            ...
            *(rt_tick_t *)arg = TIMER_FLAG_ACTIVATED;
        }
        else
        {
            /*timer is stop*/
            *(rt_tick_t *)arg = TIMER_FLAG_DEACTIVATED;
        }
        break;
...
OS_MODE_EXPORT(rt_timer_control);
...
void rt_timer_check(void)
{
    ...
    register rt_base_t level;
    rt_list_t list = OS_LIST_OBJECT_INIT(list);

    ENABLE_DEBUG_LOG(ENABLE_DEBUG_TIMER, ("timer check enter\n"));
    ...
    while (!rt_list_isempty(&rt_timer_list[TIMER_SKIP_LIST_LEVEL - 1]))
    {
        t = rt_list_entry(rt_timer_list[TIMER_SKIP_LIST_LEVEL - 1].next,
                          struct rt_timer, row[TIMER_SKIP_LIST_LEVEL - 1]);
        ...
        {
            OBJECT_HOOK_CALL(rt_timer_enter_hook, (t));

            /* remove timer from timer list firstly */
            _rt_timer_remove(t);
            if (!(t->parent.flag & TIMER_FLAG_PERIODIC))
            {
                t->parent.flag &= ~TIMER_FLAG_ACTIVATED;
            }
            ...
            OBJECT_HOOK_CALL(rt_timer_exit_hook, (t));
            ENABLE_DEBUG_LOG(ENABLE_DEBUG_TIMER, ("current tick: %d\n", current_tick));
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_TIMER, ("timer check leave\n"));
}
...
void rt_soft_timer_check(void)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_TIMER, ("software timer check enter\n"));

    /* disable interrupt */
    level = rt_hw_interrupt_disable();

    while (!rt_list_isempty(&rt_soft_timer_list[TIMER_SKIP_LIST_LEVEL - 1]))
    {
        t = rt_list_entry(rt_soft_timer_list[TIMER_SKIP_LIST_LEVEL - 1].next,
                            struct rt_timer, row[TIMER_SKIP_LIST_LEVEL - 1]);
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_TIMER, ("software timer check leave\n"));
}
...
static void rt_thread_timer_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if ((next_timeout - current_tick) < TICK_MAX / 2)
            {
