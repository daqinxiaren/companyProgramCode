 ...
 *        which shall be IPC_FLAG_FIFO/IPC_FLAG_PRIO.
 *
 * @return the operation status, OS_EOK on successful
 */
...
rt_inline rt_err_t rt_ipc_list_resume(rt_list_t *list)
{
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_IPC, ("resume thread:%s\n", thread->name));
...
rt_err_t rt_sem_init(rt_sem_t    sem,
                     ...
                     rt_uint8_t  flag)
{
    OS_ASSERT(sem != OS_NULL);
    OS_ASSERT(value < 0x10000U);

    /* initialize object */
    rt_object_init(&(sem->parent.parent), OBJECT_CLASS_SEMAPHORE, name);
...
rt_err_t rt_sem_detach(rt_sem_t sem)
{
    /* parameter check */
    OS_ASSERT(sem != OS_NULL);
    OS_ASSERT(rt_object_get_type(&sem->parent.parent) == OBJECT_CLASS_SEMAPHORE);
    OS_ASSERT(rt_object_is_systemobject(&sem->parent.parent));
 ...
 * @param flag the flag of semaphore
 *
 * @return the created semaphore, OS_NULL on error happen
 *
 * @see rt_sem_init
 */
rt_sem_t rt_sem_create(const char *name, rt_uint32_t value, rt_uint8_t flag)
{
    ...
    /* allocate object */
    sem = (rt_sem_t)rt_object_allocate(OBJECT_CLASS_SEMAPHORE, name);
    if (sem == OS_NULL)
        return sem;
...
rt_err_t rt_sem_delete(rt_sem_t sem)
{
    ...
    OS_ASSERT(sem != OS_NULL);
    OS_ASSERT(rt_object_get_type(&sem->parent.parent) == OBJECT_CLASS_SEMAPHORE);
    OS_ASSERT(rt_object_is_systemobject(&sem->parent.parent) == OS_FALSE);
...
rt_err_t rt_sem_take(rt_sem_t sem, rt_int32_t time)
{
    ...
    OBJECT_HOOK_CALL(rt_object_trytake_hook, (&(sem->parent.parent)));

    /* disable interrupt */
    temp = rt_hw_interrupt_disable();

    ENABLE_DEBUG_LOG(ENABLE_DEBUG_IPC, ("thread %s take sem:%s, which value is: %d\n",
                                rt_thread_self()->name,
    ...
    else
    {
        ...
        else
        {
            ...
            ENABLE_DEBUG_LOG(ENABLE_DEBUG_IPC, ("sem take: suspend thread - %s\n",
                                        thread->name));
            ...
            if (time > 0)
            {
                ENABLE_DEBUG_LOG(ENABLE_DEBUG_IPC, ("set thread:%s to timer list\n",
                                            thread->name));
    ...
    OBJECT_HOOK_CALL(rt_object_take_hook, (&(sem->parent.parent)));

    return OS_EOK;
}
OS_MODE_EXPORT(rt_sem_take);
...
rt_err_t rt_sem_release(rt_sem_t sem)
{
    ...
    OBJECT_HOOK_CALL(rt_object_put_hook, (&(sem->parent.parent)));
    ...
    ENABLE_DEBUG_LOG(ENABLE_DEBUG_IPC, ("thread %s releases sem:%s, which value is: %d\n",
                                rt_thread_self()->name,
                                ((struct rt_object *)sem)->name,
                                sem->value));
    ...
    {
        if(sem->value < SEMAPHORE_VALUE_MAX)
        {
        ...
        else
        {
            ...
            return -OS_EFULL; /* value overflowed */
        }
    }
...
OS_MODE_EXPORT(rt_sem_control);
#endif /* end of ENABLE_SEMAPHORE */
...
{
    /* parameter check */
    OS_ASSERT(mutex != OS_NULL);

    /* initialize object */
    rt_object_init(&(mutex->parent.parent), OBJECT_CLASS_MUTEX, name);
...
rt_err_t rt_mutex_detach(rt_mutex_t mutex)
{
    /* parameter check */
    OS_ASSERT(mutex != OS_NULL);
    OS_ASSERT(rt_object_get_type(&mutex->parent.parent) == OBJECT_CLASS_MUTEX);
    OS_ASSERT(rt_object_is_systemobject(&mutex->parent.parent));
...
OS_MODE_EXPORT(rt_mutex_detach);
 ...
 * @param flag the flag of mutex
 *
 * @return the created mutex, OS_NULL on error happen
 *
 * @see rt_mutex_init
 */
rt_mutex_t rt_mutex_create(const char *name, rt_uint8_t flag)
{
    ...
    /* allocate object */
    mutex = (rt_mutex_t)rt_object_allocate(OBJECT_CLASS_MUTEX, name);
    if (mutex == OS_NULL)
        return mutex;
...
OS_MODE_EXPORT(rt_mutex_create);
...
rt_err_t rt_mutex_delete(rt_mutex_t mutex)
{
    ...
    OS_ASSERT(mutex != OS_NULL);
    OS_ASSERT(rt_object_get_type(&mutex->parent.parent) == OBJECT_CLASS_MUTEX);
    OS_ASSERT(rt_object_is_systemobject(&mutex->parent.parent) == OS_FALSE);
...
OS_MODE_EXPORT(rt_mutex_delete);
#endif
...
rt_err_t rt_mutex_take(rt_mutex_t mutex, rt_int32_t time)
{
    ...
    OBJECT_HOOK_CALL(rt_object_trytake_hook, (&(mutex->parent.parent)));

    ENABLE_DEBUG_LOG(ENABLE_DEBUG_IPC,
...
__again:
        ...
        else
        {
            ...
            else
            {
                ...
                ENABLE_DEBUG_LOG(ENABLE_DEBUG_IPC, ("mutex_take: suspend thread: %s\n",
                                            thread->name));
                ...
                if (thread->error != OS_EOK)
                {
                    ...
                    if (thread->error == -OS_EINTR) goto __again;
#endif /* end of ENABLE_SIGNALS */
    ...
    OBJECT_HOOK_CALL(rt_object_take_hook, (&(mutex->parent.parent)));

    return OS_EOK;
}
OS_MODE_EXPORT(rt_mutex_take);
...
rt_err_t rt_mutex_release(rt_mutex_t mutex)
{
    ...
    OBJECT_HOOK_CALL(rt_object_put_hook, (&(mutex->parent.parent)));
    ...
    if (mutex->hold == 0)
    {
        ...
        if (!rt_list_isempty(&mutex->parent.suspend_thread))
        {
            ...
            ENABLE_DEBUG_LOG(ENABLE_DEBUG_IPC, ("mutex_release: resume thread: %s\n",
                                        thread->name));
        ...
        {
            if(mutex->value < MUTEX_VALUE_MAX)
            {
                /* increase value */
                mutex->value ++;
            }
...
OS_MODE_EXPORT(rt_mutex_release);
...
OS_MODE_EXPORT(rt_mutex_control);
#endif /* end of ENABLE_MUTEX */
...
rt_err_t rt_event_init(rt_event_t event, const char *name, rt_uint8_t flag)
{
    ...
    /* initialize object */
    rt_object_init(&(event->parent.parent), OBJECT_CLASS_EVENT, name);
...
rt_err_t rt_event_detach(rt_event_t event)
{
    /* parameter check */
    OS_ASSERT(event != OS_NULL);
    OS_ASSERT(rt_object_get_type(&event->parent.parent) == OBJECT_CLASS_EVENT);
    OS_ASSERT(rt_object_is_systemobject(&event->parent.parent));
...
OS_MODE_EXPORT(rt_event_detach);
 ...
 * @param flag the flag of event
 *
 * @return the created event, OS_NULL on error happen
 */
rt_event_t rt_event_create(const char *name, rt_uint8_t flag)
{
    ...
    /* allocate object */
    event = (rt_event_t)rt_object_allocate(OBJECT_CLASS_EVENT, name);
    if (event == OS_NULL)
        return event;
...
OS_MODE_EXPORT(rt_event_create);
...
rt_err_t rt_event_delete(rt_event_t event)
{
    ...
    OS_ASSERT(event != OS_NULL);
    OS_ASSERT(rt_object_get_type(&event->parent.parent) == OBJECT_CLASS_EVENT);
    OS_ASSERT(rt_object_is_systemobject(&event->parent.parent) == OS_FALSE);
...
OS_MODE_EXPORT(rt_event_delete);
#endif
...
rt_err_t rt_event_send(rt_event_t event, rt_uint32_t set)
{
    ...
    OBJECT_HOOK_CALL(rt_object_put_hook, (&(event->parent.parent)));
    ...
    if (!rt_list_isempty(&event->parent.suspend_thread))
    {
        ...
        while (n != &(event->parent.suspend_thread))
        {
            ...
            status = -OS_ERROR;
            if (thread->event_info & EVENT_FLAG_AND)
            {
            ...
            else if (thread->event_info & EVENT_FLAG_OR)
            {
            ...
            {
                /* clear event */
                if (thread->event_info & EVENT_FLAG_CLEAR)
 ...
 * @param option the receive option, either EVENT_FLAG_AND or
 *        EVENT_FLAG_OR should be set.
 * @param timeout the waiting time
 * @param recved the received event, if you don't care, OS_NULL can be set.
 *
 * @return the error code
 */
                       ...
                       rt_uint32_t *recved)
{
    ...
    OBJECT_HOOK_CALL(rt_object_trytake_hook, (&(event->parent.parent)));
    ...
    {
        /* either EVENT_FLAG_AND or EVENT_FLAG_OR should be set */
        OS_ASSERT(0);
    }
    ...
    OBJECT_HOOK_CALL(rt_object_take_hook, (&(event->parent.parent)));

    return thread->error;
}
OS_MODE_EXPORT(rt_event_recv);
...
OS_MODE_EXPORT(rt_event_control);
#endif /* end of ENABLE_EVENT */
                    ...
                    rt_uint8_t   flag)
{
    ...
    /* initialize object */
    rt_object_init(&(mb->parent.parent), OBJECT_CLASS_MAILBOX, name);
...
rt_err_t rt_mb_detach(rt_mailbox_t mb)
{
    /* parameter check */
    OS_ASSERT(mb != OS_NULL);
    OS_ASSERT(rt_object_get_type(&mb->parent.parent) == OBJECT_CLASS_MAILBOX);
    OS_ASSERT(rt_object_is_systemobject(&mb->parent.parent));
 ...
 * @param flag the flag of mailbox
 *
 * @return the created mailbox, OS_NULL on error happen
 */
rt_mailbox_t rt_mb_create(const char *name, rt_size_t size, rt_uint8_t flag)
{
    ...
    /* allocate object */
    mb = (rt_mailbox_t)rt_object_allocate(OBJECT_CLASS_MAILBOX, name);
    if (mb == OS_NULL)
        return mb;
    ...
    /* initialize mailbox */
    mb->size     = size;
    mb->msg_pool = (rt_ubase_t *)KERNEL_MALLOC(mb->size * sizeof(rt_ubase_t));
    if (mb->msg_pool == OS_NULL)
    {
...
rt_err_t rt_mb_delete(rt_mailbox_t mb)
{
    ...
    OS_ASSERT(mb != OS_NULL);
    OS_ASSERT(rt_object_get_type(&mb->parent.parent) == OBJECT_CLASS_MAILBOX);
    OS_ASSERT(rt_object_is_systemobject(&mb->parent.parent) == OS_FALSE);
                         ...
                         rt_int32_t   timeout)
{
    ...
    OBJECT_HOOK_CALL(rt_object_put_hook, (&(mb->parent.parent)));
    ...
    while (mb->entry == mb->size)
    {
        ...
        if (timeout > 0)
        {
            ...
            ENABLE_DEBUG_LOG(ENABLE_DEBUG_IPC, ("mb_send_wait: start timer of thread:%s\n",
                                        thread->name));
...
OS_MODE_EXPORT(rt_mb_send_wait);
...
rt_err_t rt_mb_recv(rt_mailbox_t mb, rt_ubase_t *value, rt_int32_t timeout)
{
    ...
    OBJECT_HOOK_CALL(rt_object_trytake_hook, (&(mb->parent.parent)));
    ...
    while (mb->entry == 0)
    {
        ...
        if (timeout > 0)
        {
            ...
            ENABLE_DEBUG_LOG(ENABLE_DEBUG_IPC, ("mb_recv: start timer of thread:%s\n",
                                        thread->name));
    ...
    *value = mb->msg_pool[mb->out_offset];
    ...
    if (!rt_list_isempty(&(mb->suspend_sender_thread)))
    {
        ...
        OBJECT_HOOK_CALL(rt_object_take_hook, (&(mb->parent.parent)));
...
OS_MODE_EXPORT(rt_mb_control);
#endif /* end of ENABLE_MAIL_BOX */
                    ...
                    rt_uint8_t  flag)
{
    ...
    /* initialize object */
    rt_object_init(&(mq->parent.parent), OBJECT_CLASS_MESSAGE_QUEUE, name);
...
rt_err_t rt_mq_detach(rt_mq_t mq)
{
    /* parameter check */
    OS_ASSERT(mq != OS_NULL);
    OS_ASSERT(rt_object_get_type(&mq->parent.parent) == OBJECT_CLASS_MESSAGE_QUEUE);
    OS_ASSERT(rt_object_is_systemobject(&mq->parent.parent));
 ...
 * @param flag the flag of message queue
 *
 * @return the created message queue, OS_NULL on error happen
 */
                     ...
                     rt_uint8_t  flag)
{
    ...
    /* allocate object */
    mq = (rt_mq_t)rt_object_allocate(OBJECT_CLASS_MESSAGE_QUEUE, name);
    if (mq == OS_NULL)
        return mq;
    ...
    mq->msg_pool = KERNEL_MALLOC((mq->msg_size + sizeof(struct rt_mq_message)) * mq->max_msgs);
    if (mq->msg_pool == OS_NULL)
    {
...
rt_err_t rt_mq_delete(rt_mq_t mq)
{
    ...
    OS_ASSERT(mq != OS_NULL);
    OS_ASSERT(rt_object_get_type(&mq->parent.parent) == OBJECT_CLASS_MESSAGE_QUEUE);
    OS_ASSERT(rt_object_is_systemobject(&mq->parent.parent) == OS_FALSE);
                         ...
                         rt_int32_t  timeout)
{
    ...
    OBJECT_HOOK_CALL(rt_object_put_hook, (&(mq->parent.parent)));
    ...
    while ((msg = mq->msg_queue_free) == OS_NULL)
    {
        /* reset error number in thread */
        thread->error = OS_EOK;
        ...
        if (timeout > 0)
        {
            ...
            ENABLE_DEBUG_LOG(ENABLE_DEBUG_IPC, ("mq_send_wait: start timer of thread:%s\n",
                                        thread->name));
    ...
    if (mq->msg_queue_head == OS_NULL)
        mq->msg_queue_head = msg;

    if(mq->entry < MQ_ENTRY_MAX)
    {
        /* increase message entry */
        mq->entry ++;
    }
...
OS_MODE_EXPORT(rt_mq_send_wait)
                    ...
                    rt_int32_t timeout)
{
    ...
    /* get current thread */
    thread = rt_thread_self();
    OBJECT_HOOK_CALL(rt_object_trytake_hook, (&(mq->parent.parent)));
    ...
    if (!rt_list_isempty(&(mq->suspend_sender_thread)))
    {
        ...
        OBJECT_HOOK_CALL(rt_object_take_hook, (&(mq->parent.parent)));
...
OS_MODE_EXPORT(rt_mq_control);
#endif /* end of ENABLE_MESSAGE_QUEUE */
