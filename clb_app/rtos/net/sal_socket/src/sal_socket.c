...
#if SYSTEM_WORKQUEUE_STACKSIZE < 1536
...
#define SAL_NETDEV_NETDBOPS_VALID(netdev, pf, ops)                                \
    ...
    ((pf) = (struct sal_proto_family *) (netdev)->sal_user_data) != OS_NULL &&    \
    (pf)->netdb_ops->ops)                                                         \
...
int sal_init(void)
{
    ...
    if (socket_table.sockets == OS_NULL)
    {
    ...
    rt_mutex_init(&sal_core_lock, "sal_lock", IPC_FLAG_FIFO);
...
#define SAL_INTERNET_DATE_LEN  16
    ...
    if (pf == OS_NULL || pf->skt_ops == OS_NULL)
    {
        result = -OS_ERROR;
        goto __exit;
    }
    ...
    send_data[9] = OS_VERSION;
    send_data[10] = OS_SUBVERSION;
    send_data[11] = OS_REVISION;
...
int sal_check_netdev_internet_up(struct netdev *netdev)
{
    ...
    struct rt_delayed_work *net_work = OS_NULL;
    ...
    rt_work_submit(&(net_work->work), TICK_PER_SECOND);
#endif /* SAL_INTERNET_CHECK */
    return 0;
}
...
struct sal_socket *sal_get_socket(int socket)
{
    ...
    OS_ASSERT(st->sockets[socket]->magic == SAL_SOCKET_MAGIC);
...
static void sal_lock(void)
{
    ...
    result = rt_mutex_take(&sal_core_lock, WAITING_FOREVER);
    if (result != OS_EOK)
    {
        OS_ASSERT(0);
    }
...
static int socket_init(int family, int type, int protocol, struct sal_socket **res)
{
    ...
    if (netdv_def && netdev_is_up(netdv_def))
    {
        ...
        if (pf != OS_NULL && pf->skt_ops && (pf->family == family || pf->sec_family == family))
        {
...
static int socket_alloc(struct sal_socket_table *st, int f_socket)
{
    ...
    if (idx < (int) st->max_socket && st->sockets[idx] == OS_NULL)
    {
...
static int socket_new(void)
{
    ...
    sock->netdev = OS_NULL;
    sock->user_data = OS_NULL;
#ifdef SAL_ENABLE_TLS
    sock->user_data_tls = OS_NULL;
#endif
...
int sal_bind(int socket, const struct sockaddr *name, socklen_t namelen)
{
    ...
    {
        struct sal_proto_family *input_pf = OS_NULL, *local_pf = OS_NULL;
        struct netdev *new_netdev = OS_NULL;

        new_netdev = netdev_get_by_ipaddr(&input_ipaddr);
        if (new_netdev == OS_NULL)
        {
            return -1;
        }
                    ...
                    struct addrinfo **res)
{
    ...
    if(ret == OS_EOK)
    {
        ...
        {
            if(sal_dev_res_tbl[i].res == OS_NULL)
            {
...
void sal_freeaddrinfo(struct addrinfo *ai)
{
    ...
    for(i = 0; i < SAL_SOCKETS_NUM; i++)
    {
        if(sal_dev_res_tbl[i].res == ai)
        {
            ...
            sal_dev_res_tbl[i].res = OS_NULL;
            sal_dev_res_tbl[i].netdev = OS_NULL;
            break;
        }
    }
    OS_ASSERT((i < SAL_SOCKETS_NUM));
