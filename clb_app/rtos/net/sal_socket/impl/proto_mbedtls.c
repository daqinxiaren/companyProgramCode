...
{
    MbedTLSSession *session = OS_NULL;
    char *pers = "mbedtls";
    ...
    if (mbedtls_client_init(session, (void *) pers, rt_strlen(pers)) != OS_EOK)
    {
...
int mbedtls_net_send_cb(void *ctx, const unsigned char *buf, size_t len)
{
    ...
    ret = pf->skt_ops->sendto((int) sock->user_data, (void *)buf, len, 0, OS_NULL, OS_NULL);
    if (ret < 0)
    {
#ifdef ENABLE_DFS
...
int mbedtls_net_recv_cb( void *ctx, unsigned char *buf, size_t len)
{
    ...
    ret = pf->skt_ops->recvfrom((int) sock->user_data, (void *)buf, len, 0, OS_NULL, OS_NULL);
    if (ret < 0)
    {
#ifdef ENABLE_DFS
        if ((fcntl(socket, F_GETFL) & O_NONBLOCK) == O_NONBLOCK)
...
static int mbedtls_connect(void *sock)
{
    ...
    mbedtls_ssl_set_bio(&session->ssl, &session->server_fd, mbedtls_net_send_cb, mbedtls_net_recv_cb, OS_NULL);
