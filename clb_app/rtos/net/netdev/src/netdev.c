...
int netdev_register(struct netdev *netdev, const char *name, void *user_data)
{
    ...
    netdev->status_callback = OS_NULL;
    netdev->addr_callback = OS_NULL;

    /* fill network interface device */
    rt_strncpy(netdev->name, name, OS_NAME_MAX);
...
int netdev_unregister(struct netdev *netdev)
{
    ...
    struct netdev *cur_netdev = OS_NULL;
    ...
    for (node = &(netdev_list->list); node; node = rt_slist_next(node))
    {
        ...
        if (cur_netdev == netdev)
        {
            ...
            if (netdev_list == netdev && rt_slist_next(&netdev_list->list) == OS_NULL)
            {
                netdev_list = OS_NULL;
            }
            else
...
struct netdev *netdev_get_by_name(const char *name)
{
    ...
    {
        netdev = rt_slist_entry(node, struct netdev, list);
        if (netdev && (rt_strncmp(netdev->name, name, OS_NAME_MAX) == 0))
        {
            rt_hw_interrupt_enable(level);
            return netdev;
        }
...
struct netdev *netdev_get_by_family(int family)
{
    ...
    rt_slist_t *node = OS_NULL;
    struct netdev *netdev = OS_NULL;
    struct sal_proto_family *pf = OS_NULL;
...
int netdev_set_ipaddr(struct netdev *netdev, const ip_addr_t *ip_addr)
{
    ...
    return netdev->ops->set_addr_info(netdev, (ip_addr_t *)ip_addr, OS_NULL, OS_NULL);
}
...
int netdev_set_netmask(struct netdev *netdev, const ip_addr_t *netmask)
{
    ...
    return netdev->ops->set_addr_info(netdev, OS_NULL, (ip_addr_t *)netmask, OS_NULL);
}
...
int netdev_set_gw(struct netdev *netdev, const ip_addr_t *gw)
{
    ...
    return netdev->ops->set_addr_info(netdev, OS_NULL, OS_NULL, (ip_addr_t *)gw);
}
...
#ifdef NETDEV_ENABLE_AUTO_DEFAULT
...
{
    struct netdev *new_netdev = OS_NULL;
...
#endif /* NETDEV_ENABLE_AUTO_DEFAULT */
...
void netdev_set_if(char* netdev_name, char* ip_addr, char* gw_addr, char* nm_addr)
{
    ...
    /* set IP address */
    if ((ip_addr != OS_NULL) && inet_aton(ip_addr, &addr))
    {
        netdev_set_ipaddr(netdev, &addr);
    }

    /* set gateway address */
    if ((gw_addr != OS_NULL) && inet_aton(gw_addr, &addr))
    {
        netdev_set_gw(netdev, &addr);
    }

    /* set netmask address */
    if ((nm_addr != OS_NULL) && inet_aton(nm_addr, &addr))
    {
...
#endif /* NETDEV_ENABLE_IFCONFIG */

#ifdef NETDEV_ENABLE_PING
int netdev_cmd_ping(char* target_name, rt_uint32_t times, rt_size_t size)
{
#define NETDEV_PING_DATA_SIZE       32
/** ping receive timeout - in milliseconds */
#define NETDEV_PING_RECV_TIMEO      (2 * TICK_PER_SECOND)
/** ping delay - in milliseconds */
#define NETDEV_PING_DELAY           (1 * TICK_PER_SECOND)
...
#define NETDEV_PING_IS_COMMONICABLE(netdev) \
    ...
    else
    {
        ...
        else if (netdev->ops == OS_NULL || netdev->ops->ping == OS_NULL)
        {
...
static void netdev_set_dns(char *netdev_name, uint8_t dns_num, char *dns_server)
{
    ...
    if (netdev_set_dns_server(netdev, dns_num, &dns_addr) == OS_EOK)
    {
