...
static at_server_t at_server_local = OS_NULL;
static at_cmd_t cmd_table = OS_NULL;
...
{
    OS_ASSERT(at_server_local);
    OS_ASSERT(at_server_local->status != AT_STATUS_UNINITIALIZED);
...
static rt_err_t at_server_gerchar(at_server_t server, char *ch, rt_int32_t timeout)
{
    ...
    {
        rt_sem_control(at_server_local->rx_notice, IPC_CMD_RESET, OS_NULL);
...
#define DELECT_KEY              0x7F
    ...
    at_cmd_t cur_cmd = OS_NULL;
    char *cur_cmd_args = OS_NULL, ch, last_ch;

    OS_ASSERT(server);
    OS_ASSERT(server->status != AT_STATUS_UNINITIALIZED);

    while (1)
    {
        server->get_char(server, &ch, WAITING_FOREVER);
        if (ESC_KEY == ch)
        {
            break;
        }
...
int at_server_init(void)
{
    ...
    at_server_local->rx_notice = rt_sem_create("at_svr", 0, IPC_FLAG_FIFO);
    ...
    {
        OS_ASSERT(at_server_local->device->type == DEVICE_CLASS_CHAR);

        /* using DMA mode first */
        open_result = rt_device_open(at_server_local->device, DEVICE_OFLAG_RDWR | DEVICE_FLAG_DMA_RX);
        /* using interrupt mode when DMA mode not supported */
        if (open_result == -OS_EIO)
        {
            open_result = rt_device_open(at_server_local->device, DEVICE_OFLAG_RDWR | DEVICE_FLAG_INT_RX);
        }
    ...
    at_server_local->parser = rt_thread_create("at_svr",
                                         ...
                                         5);
    if (at_server_local->parser == OS_NULL)
    {
        result = -OS_ENOMEM;
        goto __exit;
    }
