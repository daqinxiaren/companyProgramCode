...
AT_CMD_EXPORT("AT", OS_NULL, OS_NULL, OS_NULL, OS_NULL, at_exec);
...
AT_CMD_EXPORT("ATZ", OS_NULL, OS_NULL, OS_NULL, OS_NULL, atz_exec);
...
AT_CMD_EXPORT("AT+RST", OS_NULL, OS_NULL, OS_NULL, OS_NULL, at_rst_exec);
...
AT_CMD_EXPORT("ATE", "<value>", OS_NULL, OS_NULL, ate_setup, OS_NULL);
...
AT_CMD_EXPORT("AT&L", OS_NULL, OS_NULL, OS_NULL, OS_NULL, at_show_cmd_exec);
...
static at_result_t at_uart_setup(const char *args)
{
    ...
    if(rt_device_control(at_get_server()->device, DEVICE_CTRL_CONFIG, &config) != OS_EOK)
    {
        return AT_RESULT_FAILE;
    }
...
AT_CMD_EXPORT("AT+UART", "=<baudrate>,<databits>,<stopbits>,<parity>,<flow_control>", OS_NULL, at_uart_query, at_uart_setup, OS_NULL);
