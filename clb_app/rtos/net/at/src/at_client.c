 ...
 *
 * @return != OS_NULL: response object
 *          = OS_NULL: no memory
 */
at_response_t at_create_resp(rt_size_t buf_size, rt_size_t line_num, rt_int32_t timeout)
{
    at_response_t resp = OS_NULL;
 ...
 *
 * @return != OS_NULL: response line buffer
 *          = OS_NULL: input response line error
 */
const char *at_resp_get_line(at_response_t resp, rt_size_t resp_line)
{
    char *resp_buf = resp->buf;
    char *resp_line_buf = OS_NULL;
...
int at_resp_parse_line_args(at_response_t resp, rt_size_t resp_line, const char *resp_expr, ...)
{
    va_list args;
    int resp_args_num = 0;
    const char *resp_line_buf = OS_NULL;

    OS_ASSERT(resp);
    OS_ASSERT(resp_expr);

    if ((resp_line_buf = at_resp_get_line(resp, resp_line)) == OS_NULL)
    {
        return -1;
    }
...
int at_resp_parse_line_args_by_kw(at_response_t resp, const char *keyword, const char *resp_expr, ...)
{
    ...
    if ((resp_line_buf = at_resp_get_line_by_kw(resp, keyword)) == OS_NULL)
    {
        return -1;
    }
 ...
 * @param resp AT response object, using OS_NULL when you don't care response
...
int at_obj_exec_cmd(at_client_t client, at_response_t resp, const char *cmd_expr, ...)
{
    ...
    rt_mutex_take(client->lock, WAITING_FOREVER);
    ...
    if (resp != OS_NULL)
    {
        if (rt_sem_take(client->resp_notice, resp->timeout) != OS_EOK)
        {
...
int at_obj_set_urc_table(at_client_t client, const struct at_urc *urc_table, rt_size_t table_sz)
{
    ...
    {
        struct at_urc_table *old_urc_table = OS_NULL;
...
{
    if (at_client_table[0].device == OS_NULL)
    {
        return OS_NULL;
    }
...
static const struct at_urc *get_urc_obj(at_client_t client)
{
    ...
    rt_size_t bufsz;
    char *buffer = OS_NULL;
    const struct at_urc *urc = OS_NULL;
    struct at_urc_table *urc_table = OS_NULL;
...
static int at_recv_readline(at_client_t client)
{
    ...
    while (1)
    {
        at_client_getchar(client, &ch, WAITING_FOREVER);
...
static void client_parser(at_client_t client)
{
    ...
    while(1)
    {
        ...
        {
            if ((urc = get_urc_obj(client)) != OS_NULL)
            {
...
#define AT_CLIENT_THREAD_NAME          "at_clnt"
    ...
    if (client->recv_line_buf == OS_NULL)
    {
    ...
    rt_snprintf(name, OS_NAME_MAX, "%s%d", AT_CLIENT_LOCK_NAME, at_client_num);
    client->lock = rt_mutex_create(name, IPC_FLAG_FIFO);
    if (client->lock == OS_NULL)
    {
    ...
    rt_snprintf(name, OS_NAME_MAX, "%s%d", AT_CLIENT_SEM_NAME, at_client_num);
    client->rx_notice = rt_sem_create(name, 0, IPC_FLAG_FIFO);
    if (client->rx_notice == OS_NULL)
    {
    ...
    rt_snprintf(name, OS_NAME_MAX, "%s%d", AT_CLIENT_RESP_NAME, at_client_num);
    client->resp_notice = rt_sem_create(name, 0, IPC_FLAG_FIFO);
    if (client->resp_notice == OS_NULL)
    {
    ...
    client->urc_table = OS_NULL;
    client->urc_table_size = 0;

    rt_snprintf(name, OS_NAME_MAX, "%s%d", AT_CLIENT_THREAD_NAME, at_client_num);
    client->parser = rt_thread_create(name,
                                     (void (*)(void *parameter))client_parser,
                                     client,
                                     1024 + 512,
                                     THREAD_PRIORITY_MAX / 3 - 1,
                                     5);
    if (client->parser == OS_NULL)
    {
        result = -OS_ENOMEM;
        goto __exit;
    }
...
int at_client_init(const char *dev_name,  rt_size_t recv_bufsz)
{
    int idx = 0;
    int result = OS_EOK;
    rt_err_t open_result = OS_EOK;
    at_client_t client = OS_NULL;

    OS_ASSERT(dev_name);
    OS_ASSERT(recv_bufsz > 0);

    if (at_client_get(dev_name) != OS_NULL)
    {
        return result;
    }
    ...
    if (client->device)
    {
        OS_ASSERT(client->device->type == DEVICE_CLASS_CHAR);

        /* using DMA mode first */
        open_result = rt_device_open(client->device, DEVICE_OFLAG_RDWR | DEVICE_FLAG_DMA_RX);
        /* using interrupt mode when DMA mode not supported */
        if (open_result == -OS_EIO)
        {
            open_result = rt_device_open(client->device, DEVICE_OFLAG_RDWR | DEVICE_FLAG_INT_RX);
        }
        OS_ASSERT(open_result == OS_EOK);
