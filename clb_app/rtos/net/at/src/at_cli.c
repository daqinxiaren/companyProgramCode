...
static struct rt_ringbuffer *console_rx_fifo = OS_NULL;
static rt_err_t (*odev_rx_ind)(rt_device_t dev, rt_size_t size) = OS_NULL;

#ifdef AT_ENABLE_CLIENT
static struct rt_semaphore client_rx_notice;
static struct rt_ringbuffer *client_rx_fifo = OS_NULL;
#endif 
...
static char console_getchar(void)
{
    ...
    rt_sem_take(&console_rx_notice, WAITING_FOREVER);
...
void at_cli_init(void)
{
    ...
    rt_sem_init(&console_rx_notice, "cli_c", 0, IPC_FLAG_FIFO);

    /* create RX FIFO */
    console_rx_fifo = rt_ringbuffer_create(AT_CLI_FIFO_SIZE);
    /* created must success */
    OS_ASSERT(console_rx_fifo);
...
static char client_getchar(void)
{
    ...
    rt_sem_take(&client_rx_notice, WAITING_FOREVER);
...
#define DELECT_KEY              0x7F
    ...
    static rt_err_t (*client_odev_rx_ind)(rt_device_t dev, rt_size_t size) = OS_NULL;
    rt_base_t int_lvl;
    ...
    if (client)
    {
        ...
        rt_sem_init(&client_rx_notice, "cli_r", 0, IPC_FLAG_FIFO);
        client_rx_fifo = rt_ringbuffer_create(AT_CLI_FIFO_SIZE);

        at_client = rt_thread_create("at_cli", at_client_entry, OS_NULL, 512, 8, 8);
        if (client_rx_fifo && at_client)
        {
            ...
            while (ESC_KEY != (ch = console_getchar()))
            {
                ...
                else if (ch == '\r' || ch == '\n')
                {
                    ...
                    {
                        rt_kprintf("\n");
                        at_obj_exec_cmd(client, OS_NULL, "%.*s", cur_line_len, cur_line);
                    }
                    cur_line_len = 0;
                }
                else
