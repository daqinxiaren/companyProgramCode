...
static int nand_erase_block(uffs_Device *dev, unsigned block)
{
    ...
    res = rt_mtd_nand_erase_block(MTD_NAND_DEVICE(dev->_private), block);

    return res == OS_EOK ? UFFS_FLASH_NO_ERR : UFFS_FLASH_IO_ERR;
}
...
static int nand_check_block(uffs_Device *dev, unsigned block)
{
    ...
    res = rt_mtd_nand_check_block(MTD_NAND_DEVICE(dev->_private), block);

    return res == OS_EOK ? UFFS_FLASH_NO_ERR : UFFS_FLASH_BAD_BLK;
}
...
static int nand_mark_badblock(uffs_Device *dev, unsigned block)
{
    ...
    res = rt_mtd_nand_mark_badblock(MTD_NAND_DEVICE(dev->_private), block);

    return res == OS_EOK ? UFFS_FLASH_NO_ERR : UFFS_FLASH_IO_ERR;
}
#endif

#if (ENABLE_UFFS_ECC_MODE == UFFS_ECC_NONE) || (ENABLE_UFFS_ECC_MODE == UFFS_ECC_SOFT)
                          ...
                          int          spare_len)
{
    ...
    {
#if defined(UFFS_USE_CHECK_MARK_FUNCITON)
        OS_ASSERT(0); //should not be here
#else
        ...
        rt_mtd_nand_read(MTD_NAND_DEVICE(dev->_private),
                         page, OS_NULL, 0,
                           ...
                           int          spare_len)
{
    ...
    OS_ASSERT(UFFS_MAX_SPARE_SIZE >= dev->attr->spare_size);
    ...
    if (data == NULL && spare == NULL)
    {
        ...
        res = rt_mtd_nand_write(MTD_NAND_DEVICE(dev->_private),
                                page, OS_NULL, 0,
                                spare, dev->attr->spare_size);//dev->mem.spare_data_size
        if (res != OS_EOK)
            goto __error;
...
//  attr->total_blocks = nand->end_block - nand->start_block + 1;/* no use */
    ...
    attr->ecc_opt = ENABLE_UFFS_ECC_MODE;               /* ecc option */
    ...
    attr->layout_opt = ENABLE_UFFS_LAYOUT;              /* let UFFS do the spare layout */
}

#elif  ENABLE_UFFS_ECC_MODE == UFFS_ECC_HW_AUTO
                               ...
                               const uffs_TagStore *ts)
{
    ...
    {
        OS_ASSERT(data_len == dev->attr->page_data_size);
    ...
    if (ts != OS_NULL)
    {
        uffs_FlashMakeSpare(dev, ts, OS_NULL, (u8 *)spare);
                               ...
                               u8            *ecc_store)        //NULL
{
    ...
    res = rt_mtd_nand_read(MTD_NAND_DEVICE(dev->_private),
                           page, data, data_len, spare, spare_len);
    if (res == 0)
    ...
    if (ts != OS_NULL)
    {
        ...
        uffs_FlashUnloadSpare(dev, (const u8 *)spare, ts, OS_NULL);
