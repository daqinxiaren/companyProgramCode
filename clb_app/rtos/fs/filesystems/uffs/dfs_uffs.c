    ...
    const void *data)
{
    ...
    OS_ASSERT(rt_strlen(fs->path) < (UFFS_MOUNT_PATH_MAX - 1));
    dev = MTD_NAND_DEVICE(fs->dev_id);

    /*1. find a empty entry in partition table */
    for (index = 0; index < UFFS_DEVICE_MAX ; index ++)
    {
        if (nand_part[index].dev == OS_NULL)
            break;
    }
...
static int dfs_uffs_mkfs(rt_device_t dev_id)
{
    ...
    for (; block <= mtd->block_end; block++)
    {
        ...
        if (rt_mtd_nand_check_block(mtd, block) != OS_EOK)
        {
...
{
    rt_base_t index;
    struct rt_mtd_nand_device *mtd = MTD_NAND_DEVICE(fs->dev_id);

    OS_ASSERT(mtd != OS_NULL);
