...
{
   if (rt_mutex_init((rt_mutex_t)sem, "mutex", IPC_FLAG_FIFO) == OS_EOK)
   {
...
rt_inline void init_MUTEX_LOCKED(struct semaphore * sem)
{
   ...
   if (rt_mutex_init((rt_mutex_t)sem, "mutex", IPC_FLAG_FIFO) == OS_EOK)
   {
	   ...
	   rt_object_detach(&(((rt_mutex_t)sem)->parent.parent));
	   rt_exit_critical();
	   rt_mutex_take((rt_mutex_t)sem, WAITING_FOREVER);
	   return;
   }
   rt_exit_critical();
...
rt_inline void init_MUTEX(struct semaphore * sem)
{
	sem->mutex = rt_mutex_create("mutex", IPC_FLAG_FIFO);
}
rt_inline init_MUTEX_LOCKED(struct semaphore * sem)
{
	sem->mutex = rt_mutex_create("mutex", IPC_FLAG_FIFO);
	rt_mutex_take(sem->mutex,  WAITING_FOREVER);
}
rt_inline down(struct semaphore * sem)
{
	rt_mutex_take(sem->mutex,  WAITING_FOREVER);
...
//#define down_trylock(struct semaphore * sem)  rt_mutex_take((rt_mutex_t)sem,  WAITING_NO)
rt_inline up(struct semaphore * sem)
{
