...
jffs2_start_garbage_collect_thread(struct jffs2_sb_info *c)
{
     ...
     rt_event_init(&sb->s_gc_thread_flags, "gc_event", IPC_FLAG_FIFO);	 
     rt_mutex_init(&sb->s_lock, "gc_mutex", IPC_FLAG_FIFO);
//     rt_mutex_init(&mte->fs->syncmode, "fs_lock", IPC_FLAG_FIFO);
...
jffs2_garbage_collect_thread(unsigned long data)
{
     ...
     while(1) {
          ...
          mte=(cyg_dir *) sb->s_root;
          OS_ASSERT(mte != NULL);
//          rt_mutex_take(&mte->fs->syncmode, WAITING_FOREVER);
