		...
		unsigned char *buffer)
{
	...
	len = rt_mtd_nor_read(MTD_NOR_DEVICE(sb->s_dev), offset, buffer, size);
	if (len != size)
		return -EIO;
		...
		size_t * return_size, unsigned char *buffer)
{
	...
	len = rt_mtd_nor_write(MTD_NOR_DEVICE(sb->s_dev), offset, buffer, size);
	if (len != size)
		return -EIO;
		...
		struct jffs2_eraseblock * jeb)
{
	...
	result = rt_mtd_nor_erase_block(MTD_NOR_DEVICE(sb->s_dev), jeb->offset, c->sector_size);
	if (result != OS_EOK)
		return -EIO;
