                    ...
                    const void* data)
{
    ...
    device_partition[index].dev = MTD_NOR_DEVICE(fs->dev_id);
...
static int _find_fs(struct cyg_mtab_entry ** mte, rt_device_t dev_id)
{
    ...
    for (index = 0; index < DEVICE_PART_MAX; index++)
    {
        if (device_partition[index].dev == MTD_NOR_DEVICE(dev_id))
        {
...
static int dfs_jffs2_unmount(struct dfs_filesystem* fs)
{
    ...
    for (index = 0; index < DEVICE_PART_MAX; index++)
    {
        if (device_partition[index].dev == MTD_NOR_DEVICE(fs->dev_id))
        {
...
static int dfs_jffs2_open(struct dfs_fd* file)
{
    ...
    {
        rt_mutex_take(&jffs2_lock, WAITING_FOREVER);
...
int dfs_jffs2_init(void)
{
    ...
    /* initialize mutex */
    if (rt_mutex_init(&jffs2_lock, "jffs2lock", IPC_FLAG_FIFO) != OS_EOK)
    {
