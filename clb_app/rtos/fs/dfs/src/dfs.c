...
#if defined(ENABLE_DFS_DEVFS) && defined(ENABLE_POSIX)
#include <libc.h>
#endif
...
int dfs_init(void)
{
    ...
    rt_mutex_init(&fslock, "fslock", IPC_FLAG_FIFO);

#ifdef DFS_ENABLE_WORKDIR
    /* set current working directory */
    memset(working_directory, 0, sizeof(working_directory));
    working_directory[0] = '/';
    rt_kprintf("\tDFS working directory\t\t\t[/].\r\n");
#endif
...
#ifdef ENABLE_DFS_DEVFS
    {
        ...
        rt_kprintf("\tDEV FS working directory\t\t[/dev].\r\n");
    }
#endif

    init_ok = OS_TRUE;
    rt_kprintf("\tDFS Core init.\r\n");
    return 0;
}
...
void dfs_lock(void)
{
    rt_err_t result = -OS_EBUSY;

    while (result == -OS_EBUSY)
    {
        result = rt_mutex_take(&fslock, WAITING_FOREVER);
    }
...
OS_MODE_EXPORT(dfs_normalize_path);
