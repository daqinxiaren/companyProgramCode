...
int dfs_elm_mount(struct dfs_filesystem *fs, unsigned long rwflag, const void *data)
{
    ...
    /* check sector size */
    if (rt_device_control(fs->dev_id, DEVICE_CTRL_BLK_GETGEOME, &geometry) == OS_EOK)
    {
...
int dfs_elm_unmount(struct dfs_filesystem *fs)
{
    ...
    logic_nbr[0] = '0' + index;
    result = f_mount(OS_NULL, logic_nbr, (BYTE)0);
    if (result != FR_OK)
        return elm_result_to_dfs(result);
...
#define FSM_STATUS_USE_TEMP_DRIVER 1
    ...
    if (index == -1)
    {
        ...
        else
        {
            ...
            disk[index] = dev_id;
            /* try to open device */
            rt_device_open(dev_id, DEVICE_OFLAG_RDWR);
...
DRESULT disk_ioctl(BYTE drv, BYTE ctrl, void *buff)
{
    ...
    if (ctrl == GET_SECTOR_COUNT)
    {
        ...
        rt_device_control(device, DEVICE_CTRL_BLK_GETGEOME, &geometry);
    ...
    {
        rt_device_control(device, DEVICE_CTRL_BLK_SYNC, OS_NULL);
    }
    else if (ctrl == CTRL_TRIM)
    {
        rt_device_control(device, DEVICE_CTRL_BLK_ERASE, buff);
    }
...
int ff_cre_syncobj(BYTE drv, FF_SYNC_t *m)
{
    ...
    mutex = rt_mutex_create(name, IPC_FLAG_FIFO);
    if (mutex != OS_NULL)
    {
        *m = mutex;
        return OS_TRUE;
...
{
    if (rt_mutex_take(m, FF_FS_TIMEOUT) == OS_EOK)
        return OS_TRUE;
