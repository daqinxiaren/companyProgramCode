...
int gettimeofday(struct timeval *tp, void *ignore)
{
    ...
    rt_device_control(device, DEVICE_CTRL_RTC_GET_TIME, &time);
    if (tp != OS_NULL)
    {
        tp->tv_sec = time;
        tp->tv_usec = 0;
...
#endif
{
...
#ifdef ENABLE_RTC
    static rt_device_t device = OS_NULL;
    ...
    if (device != OS_NULL)
    {
        if (rt_device_open(device, 0) == OS_EOK)
        {
            rt_device_control(device, DEVICE_CTRL_RTC_GET_TIME, &time_now);
