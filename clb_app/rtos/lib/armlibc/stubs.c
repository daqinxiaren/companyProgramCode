...
int _sys_write(FILEHANDLE fh, const unsigned char *buf, unsigned len, int mode)
{
    ...
    if ((fh == STDOUT) || (fh == STDERR))
    {
#if !defined(ENABLE_CONSOLE) || !defined(ENABLE_DEVICE)
        return 0;
#else
#ifdef ENABLE_POSIX
...
#if defined(ENABLE_FINSH) && defined(FINSH_ENABLE_MSH) && defined(ENABLE_MODULE) && defined(ENABLE_DFS)
