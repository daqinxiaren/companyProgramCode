...
static int minilzo_compress_file(int fd_in, int fd_out)
{
	...
	rt_uint8_t *cmprs_buffer = OS_NULL, *buffer = OS_NULL;
...
//int minilzo_compress_data(uint8_t * in_buff, uint8_t * out_buff)
//{
//	/* Start to compress file  */
//	rt_uint8_t *cmprs_buffer = OS_NULL, *buffer = OS_NULL;
//	rt_uint8_t buffer_hdr [BLOCK_HEADER_SIZE] ={ 0 };
...
static int minilzo_decompress_file(int fd_in, int fd_out)
{
	...
	rt_uint8_t *dcmprs_buffer = OS_NULL, *buffer = OS_NULL;
	rt_uint8_t buffer_hdr [BLOCK_HEADER_SIZE] =
	{
	        0 };
